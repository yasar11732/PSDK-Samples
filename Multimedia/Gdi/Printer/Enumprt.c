  ...
  for (i = 0; i < iMaxEntries; i++)
    ...
    if (dwValue & (pLkUp + i)->dwValue)
    {
      if (bNewLine)
      {
        ...
        //
        lstrcpyn (buf, BLANKS, BUFSIZE);
        strncat (buf, (pLkUp + i)->szValue, BUFSIZE - strlen(buf));
      }
      else
      {
        //
...
BOOL DisplayPrinterDriversInfo (HWND hwnd)
{
  ...
  {
    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[0]) > 0 )
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[1], (pDriverInfo1 + i)->pName) > 0)
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[2]) > 0 )
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[3], (pDriverInfo2 + i)->cVersion) > 0 )
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[4], (pDriverInfo2 + i)->pName) > 0 )
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[5], (pDriverInfo2 + i)->pEnvironment) > 0 )
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[6], (pDriverInfo2 + i)->pDriverPath) > 0 )
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[7], (pDriverInfo2 + i)->pDataFile) > 0 )
		outstr3();

    if ( _snprintf (buf, BUFSIZE - 1, gaDriverInfo[8], (pDriverInfo2 + i)->pConfigFile) > 0 )
		outstr3();
