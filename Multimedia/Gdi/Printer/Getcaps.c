                                         ...
                                         LPARAM lParam)
{
  switch (msg)
  {
    case WM_INITDIALOG:
    {
      ...
      {
        if ( _snprintf (buf, BUFSIZE-1, "GetDeviceCaps: %s;%s;%s", gszDeviceName, gszPort, gszDriverName) > 0 )
...
BOOL DisplayDeviceCapsInfo ()
{
	...
	else
	{
		if (!(hdc = CreateDC (gszDriverName, gszDeviceName, gszPort, NULL)))
		{
			...
			if (_snprintf (buf, BUFSIZE-1, GetStringRes(IDS_FMT_CREDCFAIL),	gszDriverName, gszDeviceName, gszPort) > 0)
...
void TranslateDeviceCaps (int arrayIndex, int capIndex, int iValue)
{
  ...
  lstrcpyn (buf, gaCaps[arrayIndex].szValue, BUFSIZE);
  ...
  switch (capIndex)
  {
    case TECHNOLOGY:
    {
      ...
      for (i = 0; i < MAX_TECHNOLOGY_CAPS; i++)
        ...
        {
          strncat(buf, (gaTechnologyCaps + i)->szValue, BUFSIZE - strlen(buf));
  ...
  for (i = 0; i < iMaxEntries; i++)
    ...
    if (iValue & (pLkUp + i)->iValue)
    {
      if (bNewLine)
      {
        ...
        lstrcpyn (pbuf, BLANKS, iBuffSize);
        strncat (pbuf, (pLkUp + i)->szValue, iBuffSize - strlen(pbuf));
      }
      else
      {
        //
