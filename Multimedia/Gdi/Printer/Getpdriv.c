                                          ...
                                          WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    case WM_INITDIALOG:
    {
      ...
      {
        if ( _snprintf (buf, BUFSIZE-1, "GetPrinterDriver: %s;%s;%s", gszDeviceName, gszPort, gszDriverName) > 0)
...
BOOL DisplayPrinterDriverInfo (HWND hwnd)
{
  ...
  if (!hPrinter)
  {
    ...
    _snprintf (buf, BUFSIZE-1, GetStringRes(IDS_FMT_OPNPRTFAIL), gszDeviceName);
  ...
  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[0]) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[1], pDriverInfo1->pName) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[2]) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[3], pDriverInfo2->cVersion) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[4], pDriverInfo2->pName) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[5], pDriverInfo2->pEnvironment) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[6], pDriverInfo2->pDriverPath) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[7], pDriverInfo2->pDataFile) > 0 )
	outstr();

  if ( _snprintf (buf, BUFSIZE-1, gaDrvInfo[8], pDriverInfo2->pConfigFile) > 0 )
	outstr();
