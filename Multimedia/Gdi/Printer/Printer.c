...
LRESULT CALLBACK MainWndProc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (msg)
  {
        ...
        case ID_COMBOBOX:
          ...
          switch (HIWORD(wParam))
          {
            case CBN_SELCHANGE:
            {
              ...
              {
                lstrcpyn (gszDeviceName, "Display", BUFSIZE);
    ...
    case WM_PAINT:
    {
      ...
      _snprintf (buf, BUFSIZE -1, "cxClient = %ld (%ld)", rect.right, p.x);
      TextOut (ps.hdc, iComboboxWidth + (int) 3*glcyMenu/2, (int) glcyMenu/8,
               buf, (int) strlen (buf));
      _snprintf (buf, BUFSIZE -1, "cyClient = %ld (%ld)",
...
void RefreshPrinterCombobox (HWND hwndCombobox)
{
  ...
  if (cReturned > 0)
    ...
    for (i = 0; i < cReturned; i++)
    {
      ...
      lstrcpyn (buf, (pPrinters + i)->pPrinterName, BUFSIZE);
      strncat (buf, ";", BUFSIZE - strlen(buf));
      strncat (buf, (pPrinters + i)->pPortName, BUFSIZE - strlen(buf));
      strncat (buf, ";", BUFSIZE - strlen(buf));
      strncat (buf, (pPrinters + i)->pDriverName, BUFSIZE - strlen(buf));
