...
void SetDlgItemValue (HWND hDlg, int nIDDlgItem, int wValue, BOOL bSigned, int nBase)
{
    ...
    switch (nBase) {
        case 8:
            if (_snprintf (szValue, sizeof(szValue) - 1, "0o%o", wValue) > 0)
            {
            ...
            break;
        case 16:
            if (_snprintf (szValue, sizeof(szValue) - 1, "0x%x", wValue) > 0 )
            {
                szValue[sizeof(szValue) - 1] = '\0';
                SetDlgItemText (hDlg, nIDDlgItem, szValue);
            }
...
BOOL FAR PASCAL DlgEnumFontSizes (LPLOGFONT lpLogFont, LPTEXTMETRIC lpTextMetric, short nFontType, LPHANDLE lpData)
{
    HANDLE hFonts;
    LPFONTSTRUCT pFS;
    LPFONTLIST pFL;
...
BOOL FAR PASCAL DlgEnumFontNames (LPLOGFONT lpLogFont, LPTEXTMETRIC lpTextMetric, short nFontType, LPHANDLE lpData)
{
    HDC  hdc;
    HWND hwnd;
    HANDLE hInst;
...
static char szFacename[LF_FACESIZE];
    ...
    switch (msg) {
        case WM_INITDIALOG:
            ...
            if (_snprintf (szBuffer, sizeof(szBuffer) - 1, "TextMetrics: %s", (LPSTR)szFacename) > 0 )
            {
        ...
        case WM_COMMAND:
            ...
            switch (wmId) {
                case IDOK:
                    ...
                    lstrcpyn(lfDlg.lfFaceName, szFacename, LF_FACESIZE);
...
BOOL APIENTRY EnumDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        case WM_INITDIALOG:
            ...
            if (_snprintf (szTmp, sizeof(szTmp) - 1, "EnumFonts %d of %d", iLoc+1, count) > 0)
            {
