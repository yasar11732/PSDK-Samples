                     ...
                     LPSTR lpCmdLine, int nCmdShow)
{
    ...
    hicon = (HICON) (DWORD_PTR) GetClassLongPtr (hwndMain, GCLP_HICON);
    SetClassLongPtr (hwndTransform, GCLP_HICON, (LONG) (DWORD_PTR) hicon);
    showTransform = TRUE;
  ...
  switch (message) {
    ...
    *   fill the entry fields with the XFORM values.
    ...
    case WM_PUTUPFLOATS: {
      ...
      _snprintf (buffer, MAXCHARS -1, FORMATFLOAT,pxform->eM11);
      SetDlgItemText(hwnd, IDD_EM11, buffer);
      _snprintf (buffer, MAXCHARS - 1, FORMATFLOAT,pxform->eM12);
      SetDlgItemText(hwnd, IDD_EM12, buffer);
      _snprintf (buffer, MAXCHARS - 1, FORMATFLOAT,pxform->eDx);
      SetDlgItemText(hwnd, IDD_EDX, buffer);

      _snprintf (buffer, MAXCHARS - 1, FORMATFLOAT,pxform->eM21);
      SetDlgItemText(hwnd, IDD_EM21, buffer);
      _snprintf (buffer, MAXCHARS - 1, FORMATFLOAT,pxform->eM22);
      SetDlgItemText(hwnd, IDD_EM22, buffer);
      _snprintf (buffer, MAXCHARS - 1, FORMATFLOAT,pxform->eDy);
  ...
  switch (message) {
    ...
    *           (c.f. WM_MOUSEMOVE)
    ...
    case WM_PUTUPFLOATS: {
      ...
      _snprintf (buffer, MAXCHARS - 1, "%d", pScreen.x);
      SetDlgItemText(hwnd, IDD_DEVICEX, buffer);
      _snprintf (buffer, MAXCHARS - 1, "%d", pScreen.y);
      SetDlgItemText(hwnd, IDD_DEVICEY, buffer);

      ClientToScreen (hwndM, &pScreen);
      _snprintf (buffer, MAXCHARS - 1, "%d", pScreen.x);
      ...
      _snprintf (buffer, MAXCHARS - 1, FORMATFLOAT, (float) pWorld.x);
      SetDlgItemText(hwnd, IDD_WORLDX , buffer);
      _snprintf (buffer, MAXCHARS - 1, FORMATFLOAT, (float) pWorld.y);
...
PTrackObject doTrackObject(PTrackObject pto, int msg, HWND hwnd, LONG_PTR lParam)
{
  ...
  switch (msg) {
    ...
    * return - pointer to the new object.
    ...
    case  TROB_NEW: {
        ...
        ptobj = (PTrackObject) LocalAlloc (LPTR, sizeof (TrackObject));
		if (!ptobj)
			return NULL;

        /* initialize the HDC and other fields. */
        ptobj->hdc = GetDC(hwnd);
        SetGraphicsMode (ptobj->hdc, GM_ADVANCED);
        SetROP2(ptobj->hdc, R2_NOT);
        SelectObject (ptobj->hdc, GetStockObject (NULL_BRUSH));
        SelectObject(ptobj->hdc, CreatePen (PS_SOLID, 2, (COLORREF) 0x01000009));
        ptobj->Mode = TMNONE;
        doTrackObject (ptobj, TROB_CENTER, hwnd, lParam);
        GetWorldTransform (ptobj->hdc, &(ptobj->xfmChange));

        /* initialize the size. */
        ptobj->rect.top = ptobj->rect.left = 0;
        ptobj->rect.bottom = ptobj->rect.right = TICKSPACE*5;
    ...
    *  free up the memory allocated for the object.
    ...
    case  TROB_DELETE:
		...
		if (pto)
			LocalFree (LocalHandle ((LPSTR)pto));		/* Only free the memory if the pointer is valid */
    return NULL;
  ...
  return NULL;		/* Default option; do nothing */
}
