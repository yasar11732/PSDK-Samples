        ...
        LPSTR szJobName)
{
    ...
    if (hPrnDC = GetPrinterDC())
    {
        ...
        lstrcpyn(szJobNameTrunc, szJobName, sizeof(szJobNameTrunc) - 1);
        szJobNameTrunc[31] = '\0';           // Truncate string to 31 chars
        LoadString(ghDLLInst, IDS_PRINTMSG, lpBuffer, sizeof(lpBuffer) );
        _snprintf(szBuffer, sizeof(szBuffer) - 1, lpBuffer, (LPSTR)szJobNameTrunc);
...
//
                ...
                (GetVersion() & 0x80000000) && (LOWORD(GetVersion()) == 3))
        {
                ...
                IsRectEmpty(&rect))
            {
                ...
                _snprintf(szTmpBuf, sizeof(szTmpBuf) - 1, lpBuffer, ++nBandCount);
				szTmpBuf[sizeof(szTmpBuf) - 1] = '\0';
...
int CALLBACK PrintAbortDlg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        case WM_INITDIALOG:
        {
            ...
            _snprintf(szBuffer, sizeof(szBuffer) - 1, msgBuffer, (LPSTR)gszDevice,
                    (LPSTR)gszOutput);
