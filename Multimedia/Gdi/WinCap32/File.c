...
HDIB LoadDIB(LPSTR lpFileName)
{
    ...
    hFile = CreateFile(lpFileName, GENERIC_READ, FILE_SHARE_READ, NULL,
            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN | SECURITY_ANONYMOUS | SECURITY_SQOS_PRESENT, /* security level checks */
            NULL);
    
	if (INVALID_HANDLE_VALUE == hFile)		//Unable to open File, quit load
		return NULL;
