   ...
   LPARAM lParam)
{
	...
	//
	{
	    ...
	    if (IsNT())
            {
                _snprintf(szTip, sizeof(szTip) - 1, 
                          "%d x %d, %d bit color at %d hz",
             ...
             // Accept 0 or 1 frequency which represent default frequencies
        {
            ...
            if (bAddDM)
            {
                ...
                if (i == 0)
                {
					...
					gpCurrentMode->dmSize = (WORD)structSize;
                }
                else
                {
                    gpCurrentMode += 1;
    ...
    HWND hwnd)
{
	...
	{
        _snprintf(szRes, sizeof(szRes) - 1,
                 "%d x %d, %d bit color, default frequency",
