...
HRESULT CParamsManager::InitParams(DWORD cTimeFormats, const GUID *pguidTimeFormats, 
                                   DWORD dwFormatIndex, MP_TIMEDATA mptdTimeData, 
                                   DWORD cParams, ParamInfo *pParamInfo)
{
    ...
    {
        memcpy(&m_pguidTimeFormats[dwIndex], &pguidTimeFormats[dwIndex], 
               sizeof(*pguidTimeFormats));
    }
...
HRESULT CParamsManager::GetParamText(DWORD dwParamIndex,WCHAR **ppwchText)
{
    ...
    if (dwParamIndex < m_cParams)
    {
        ...
        // begin writing unit text here
        int iUnit = int(wcslen(pinfo.MParamInfo.szLabel)) + 1; 

        // begin writing enum text here
        int iEnums = iUnit + int(wcslen(pinfo.MParamInfo.szUnitText)) + 1; 

        // write the final (second) null terminator here
        int iEnd = iEnums + int(wcslen(pinfo.pwchText)) + 1; 

        WCHAR *pwsz = static_cast<WCHAR *>(CoTaskMemAlloc((iEnd + 1) * sizeof(WCHAR)));
        if (!pwsz)
            return E_OUTOFMEMORY;

        // wcscpy will write into various points of the string, 
        // neatly terminating each with a null
...
HRESULT CParamsManager::CopyParamsFromSource( CParamsManager * pSource)
{
    ...
    hr = InitParams(pSource->m_cTimeFormats, pSource->m_pguidTimeFormats, dwIndex, 
                    pSource->m_mptdCurrentTimeData, pSource->m_cParams,
                    pSource->m_pParamInfos);
    if (SUCCEEDED(hr))
    {
...
HRESULT CParamsManager::GetParamFloat(DWORD dwParamIndex, REFERENCE_TIME rtTime,
                                      float *pval)
{
    HRESULT hr = S_OK;
    ...
    for (CCurveItem *pCurve = pCurveHead; 
         pCurve && pCurve->m_Envelope.rtStart > rtTime;
         pCurve = pCurve->GetNext());
