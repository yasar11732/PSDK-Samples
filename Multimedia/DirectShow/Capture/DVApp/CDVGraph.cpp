...
HRESULT CDVGraph::MakeDvToFileGraph_NoPre_Type1(TCHAR* OutputFileName)
{
    ...
    hr = m_pCaptureGraphBuilder->SetOutputFileName(&MEDIASUBTYPE_Avi, T2W(OutputFileName), 
                                                   &ppf, &pSink);
...
HRESULT CDVGraph::NukeFilters(IBaseFilter *pFilter, BOOL bNukeDownStream)
{
    ...
    CHECK_ERROR( TEXT("CAVCGraph::NukeFilters():: Could not enumerate pins on the filter to be nuked.hr=%#x"), hr);
    pEnumPins->Reset(); 
