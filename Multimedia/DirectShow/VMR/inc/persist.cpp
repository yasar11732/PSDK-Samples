...
//
//       This file is #included into another source file for simplicity.
//
    ...
    )
{
    ...
    {
        lstrcpyn(szProfileString, szDefault, cb);
    ...
    )
{
    ...
    for(i = 1; i <= MAX_RECENT_FILES; i++)
    {
        ...
        hr = StringCchPrintf(szKey, NUMELMS(szKey), TEXT("File %d\0"), i);
        ...
        lstrcpyn(aRecentFiles[i - 1], FileName, NUMELMS(aRecentFiles[i-1]));
        hr = StringCchPrintf(szMenuName, NUMELMS(szMenuName), TEXT("&%d %s\0"), i, FileName);
    ...
    )
{
    ...
    lstrcpyn(aRecentFiles[0], FullPathFileName, NUMELMS(aRecentFiles[0]));
