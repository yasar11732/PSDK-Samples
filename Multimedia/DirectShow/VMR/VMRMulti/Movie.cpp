...
void CMovie::Initialize( sMovieInfo * pMovInf, CMultiSAP * pSAP)
{
    ...
    HRESULT hr = StringCchCopyW(m_achPath, NUMELMS(m_achPath), pMovInf->achPath);
...
HRESULT CMovie::AddVideoMixingRendererToFG()
{
    ...
    if(SUCCEEDED(hRes))
    {
        ...
        hRes = StringCchPrintfW( wcFilterName, NUMELMS(wcFilterName), L"VMR for VMRMulti 0x%08x\0", m_dwUserID);
