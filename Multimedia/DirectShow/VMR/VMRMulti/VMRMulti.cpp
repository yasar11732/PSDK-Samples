...
void OnButtonAddsrc(HWND hwnd)
{
    ...
    if( ListBox_GetCount(hwndList) >= MaxNumberOfMovies )
    {
        ...
        hr = StringCchPrintf(szMsg, NUMELMS(szMsg), _T("This application supports up to %d open movies.\0"),
                 MaxNumberOfMovies );
        MessageBox(hwnd, szMsg, szAppTitle, MB_OK);
        return;
    }

    hr = StringCchPrintf(szBuffer, NUMELMS(szBuffer), TEXT(""));
    ...
    if (GetOpenFileName (&ofn))  // user specified a file
    {
        ...
        hr = StringCchPrintf(szUserID, NUMELMS(szUserID), _T("0x%08x\t%s\0"), psMovInfo->pdwUserID, T2W(psMovInfo->achPath));
#else
        MultiByteToWideChar(CP_ACP, 0, (const char*)ofn.lpstrFile, -1, psMovInfo->achPath, MOVIEINFO_PATH_CHARS);
        hr = StringCchPrintf( szUserID, NUMELMS(szUserID), _T("0x%08x\t%S\0"), psMovInfo->pdwUserID,  psMovInfo->achPath);
#endif
...
void VMRMulti_OnTimer( HWND hwnd, UINT id )
{
    ...
    HRESULT hr = StringCchPrintf(WindowText, NUMELMS(WindowText),
