                           ...
                           )
{
    ...
    *lplpSurface = NULL;
    ...
    if( SUCCEEDED(hr))
    {
        ...
        ddsdTexture.ddpfPixelFormat.dwRGBBitCount = ddsdDisplay.ddpfPixelFormat.dwRGBBitCount;
		ddsdTexture.ddpfPixelFormat.dwRBitMask = ddsdDisplay.ddpfPixelFormat.dwRBitMask;
		ddsdTexture.ddpfPixelFormat.dwGBitMask = ddsdDisplay.ddpfPixelFormat.dwGBitMask;
		ddsdTexture.ddpfPixelFormat.dwBBitMask = ddsdDisplay.ddpfPixelFormat.dwBBitMask;

		/*
		if( 32 == ddsdTexture.ddpfPixelFormat.dwRGBBitCount )
		{
		...
		else if( 16 == ddsdTexture.ddpfPixelFormat.dwRGBBitCount )
		{
			ddsdTexture.ddpfPixelFormat.dwRBitMask = 0x7c00;
			ddsdTexture.ddpfPixelFormat.dwGBitMask = 0x03e0;
			ddsdTexture.ddpfPixelFormat.dwBBitMask = 0x001f;
		}
		*/
        ...
        if( FAILED(hr))
        {
            ...
            {
#ifdef SPARKLE
                (void)StringCchCopy( m_achErrorMessage, NUMELMS(m_achErrorMessage), TEXT("Direct3D object returned error code D3DERR_OUTOFVIDEOMEMORY,\r\n")\
                                             ...
                                             TEXT("or try a different video driver."));
#else
                (void)StringCchCopy( m_achErrorMessage, NUMELMS(m_achErrorMessage), TEXT("Direct3D object returned error code D3DERR_OUTOFVIDEOMEMORY,\r\n")\
                                             TEXT("which means there is not enough video memory to render this movie.\r\n")\
                                             TEXT("Naturally, this sample is supposed to demonstrate DirectShow capabilities\r\n")\
                                             TEXT("on high-end video cards. We are sorry for inconvenience.\r\n")\
                                             TEXT("You may want to try some video of a smaller size, or try a different video driver."));
#endif
                (void)StringCchCopy( m_achErrorTitle, MAX_PATH, TEXT("Out of video memory"));
            ...
            else
            {
...
#else
                hr = StringCchPrintfA( m_achErrorMessage, NUMELMS(m_achErrorMessage), TEXT("Direct3D object returned error code 0x%08x.\r\n"), hr);
#endif
                ...
                (void)StringCchCopy( m_achErrorTitle, MAX_PATH, TEXT("Direct3D error when allocating surface"));
                m_bErrorMessage = true;
            }
                                    ...
                                    DDSURFACEDESC2* pddsdDisplay    )
{
    ...
    else
    {
            ...
            pddsdDisplay->ddpfPixelFormat.dwRGBBitCount <= lpHdr->biBitCount)
        {
			...
			{
				pBitMasks = (const DWORD *)((LPBYTE)lpHdr + lpHdr->biSize);
			}
			else
			{
				switch (lpHdr->biBitCount)
				{
				case 16:
					pBitMasks = bits555;
					break;
