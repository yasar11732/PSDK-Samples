...
const TCHAR* D3DUtil_GetDXSDKMediaPath()
{
    ...
    RegCloseKey( hKey );
    HRESULT hr = StringCchCat( strPath, NUMELMS(strPath), _T("\\D3DIM\\Media\\") );
...
{
    TCHAR buffer[256];
    HRESULT hrT = StringCchPrintf( buffer, NUMELMS(buffer),_T("%hs(%ld): "), strFile, dwLine );
    OutputDebugString( buffer );
    OutputDebugString( strMsg );

    if( hr != (HRESULT) S_OK )
    {
        hr = StringCchPrintf( buffer, NUMELMS(buffer),_T("(hr=%08lx)\n"), hr );
        OutputDebugString( buffer );
    }
