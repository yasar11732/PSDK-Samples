...
bool FindMediaFile(TCHAR * achFileName, TCHAR * achFoundFile, size_t len);
    ...
    )
{
    ...
    if( !FindMediaFile(achFileName, achFoundFile, NUMELMS(achFoundFile)) )
    {
    ...
    if (pMovie) 
    {
        ...
        else 
        {
            ...
            {
                hr = StringCchPrintf(achErrorMessage, NUMELMS(achErrorMessage), TEXT("Failed to open the selected file (hr=0x%x)\r\n")\
...
bool FindMediaFile(TCHAR * achFileName, TCHAR * achFoundFile, size_t len)
{
    ...
    if( -1 != lFindRes )
    {
        hr = StringCchCopy(achFoundFile, len, achFileName);
        return true;
    }
    ...
    hr = StringCchCopy(szBuffer, NUMELMS(szBuffer), TEXT(""));
