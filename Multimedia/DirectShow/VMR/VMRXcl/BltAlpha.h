...
public:
        ...
        m_fSquare(false)
    {
        ...
        pVMenu[2].x = -0.5F;
        pVMenu[2].y = 480.f - 0.5F;;
        pVMenu[2].z = 0.5F;
        pVMenu[2].rhw = 2.0f;
        pVMenu[2].clr = RGBA_MAKE(0xff, 0xff, 0xff, 0xff);

        pVMenu[3].x = 60.f - 0.5F;
        pVMenu[3].y = 480.f - 0.5F;
        pVMenu[3].z =  0.5F;
        pVMenu[3].rhw = 2.0f;
        ...
        m_ptxtrMenu->m_pddsSurface->GetSurfaceDesc(&ddsd);

        float fMenuWidth = (float)m_ptxtrMenu->m_dwWidth   / (float)ddsd.dwWidth;
        float fMenuHeight = (float)m_ptxtrMenu->m_dwHeight / (float)ddsd.dwHeight;

        pVMenu[0].tu = 0.f;
        pVMenu[0].tv = 0.f;

        pVMenu[1].tu = fMenuWidth;
        pVMenu[1].tv = 0.f;

        pVMenu[2].tu = 0.f;
        pVMenu[2].tv = fMenuHeight;
   ...
   ~CAlphaBlt()
    ...
    // a static counter of images from CMovie:PresentImage function
    //
    // Parameters:
    //          lpDst   - destination rectangle
    //          lpSrc   - source rectangle
    //          alpha   - alpha level for the surface
    //          fWid    - width of the surface obtained by CMovie:PresentImage
    //          fHgt    - height of the surface obtained by CMovie:PresentImage
    //----------------------------------------------------------------------------
             ...
             )
    {
        ...
        __try {
            ...
            if( g_ss.bShowHelp )
            {
                hr = m_pFont->DrawText((float)g_ss.nXHelp, (float)g_ss.nYHelp,
