...
//-----------------------------------------------------------------------------
static TCHAR  g_strTexturePath[MAX_PATH] = _T("\0"); // Path for files
...
{
    lstrcpyn( m_strName, strName, NUMELMS(m_strName) );
    m_dwWidth     = 0;
    m_dwHeight    = 0;
    m_dwStage     = dwStage;
    m_dwBPP       = 0;
...
HRESULT TextureContainer::LoadImageData()
{
    ...
    int nStrLength = lstrlen(m_strName);
    lstrcpyn(strPathname, g_strTexturePath, MAX_PATH-nStrLength );
    HRESULT hr = StringCchCat( strPathname, NUMELMS(strPathname), m_strName );

    hr = StringCchPrintfA(szAnsiName, NUMELMS(szAnsiName),"%s\0", strPathname);

    if( NULL == ( file = _lopen(szAnsiName, OF_READ) ) )
    {
        // Then check if the file exists in the DirectX SDK media path
        lstrcpyn(strPathname, D3DUtil_GetDXSDKMediaPath(), NUMELMS(strPathname)-nStrLength );
        hr = StringCchCat( strPathname, NUMELMS(strPathname),m_strName );
...
HRESULT TextureContainer::CopyBitmapToSurface()
{
    ...
    RECT rcTmp = {0, 0, bm.bmWidth, bm.bmHeight};
    m_pddsSurface->Blt(&rcTmp, pddsTempSurface, &rcTmp, DDBLT_WAIT, NULL );
    ...
    if( ddsd.ddpfPixelFormat.dwRGBAlphaBitMask )
    {
        if( m_dwFlags & (D3DTEXTR_TRANSPARENTWHITE|D3DTEXTR_TRANSPARENTBLACK) )
        {
            ...
            while( m_pddsSurface->Lock( NULL, &ddsdAlpha, 0, NULL ) ==
            ...
            for( DWORD y=0; y<ddsdAlpha.dwHeight; y++ )
            {
                ...
                for( DWORD x=0; x<ddsdAlpha.dwWidth; x++ )
                {
                    ...
                    if( ddsdAlpha.ddpfPixelFormat.dwRGBBitCount == 32 )
                    {
...
VOID D3DTextr_SetTexturePath( TCHAR* strTexturePath )
{
    ...
    lstrcpyn( g_strTexturePath, strTexturePath, NUMELMS(g_strTexturePath) );
}
...
HRESULT D3DTextr_CreateEmptyTexture( TCHAR* strName, DWORD dwWidth,
                                     ...
                                     DWORD dwFlags,
                                     TextureContainer ** ppEmptyTexture )
{
    // Check parameters
    if(( NULL == strName ) || ( NULL == ppEmptyTexture ))
        return E_INVALIDARG;
