...
** -------------------------------------------------------------------------
*/
const TCHAR szClassName[] = TEXT("VMRXCLPlayer_CLASS\0");
const TCHAR g_szNULL[]    = TEXT("\0");
const TCHAR g_szEmpty[]   = TEXT("");
const DWORD g_Style       = WS_POPUP | WS_CAPTION | WS_SYSMENU;
    ...
    )
{
    ...
    */
    ...
    // Look for options
    while(lpCmdLine && (*lpCmdLine == _T('-') || *lpCmdLine == _T('/')))
    {
        if(lpCmdLine[1] == _T('P'))
        {
            g_bPlay = TRUE;
...
RepositionMovie(HWND hwnd)
{
    if(pMovie)
    {
        IVMRWindowlessControl* lpDefWC = pMovie->GetWLC();
    ...
    LPARAM lParam)
{
    ...
    g_ss.nXHelp = 0;
    g_ss.nYHelp = 0;
    HRESULT hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp),TEXT(""));
    ...
    LPARAM lParam)
{
    ...
    OutputDebugString(TEXT("*** Event is captured\n"));
    ...
    if(g_ss.bShowHelp)
    {
        OutputDebugString(TEXT("*** WILL show help\n"));
    }
    else
    {
        OutputDebugString(TEXT("*** WILL NOT show help\n"));
    }
    ...
    switch(nMenuItem)
    {
        case evxMovieRotateZ:
            g_ss.nXHelp = 70;
            g_ss.nYHelp = yPos;
            hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp), TEXT("Rotate around Z axis "));
            break;
        case evxMovieRotateY:
            g_ss.nXHelp = 70;
            g_ss.nYHelp = yPos;
            hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp), TEXT("Rotate around Y axis "));
            break;
        case evxMovieTwist:
            g_ss.nXHelp = 70;
            g_ss.nYHelp = yPos;
            hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp), TEXT("Non-linear transformation "));
            break;
        case evxExit:
            g_ss.nXHelp = 70;
            g_ss.nYHelp = yPos;
            hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp), TEXT("Exit application "));
            break;
        case evxMoviePause:
            g_ss.nXHelp = 70;
            g_ss.nYHelp = yPos;
            hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp), TEXT("Pause "));
            break;
        case evxMoviePlay:
            g_ss.nXHelp = 70;
            g_ss.nYHelp = yPos;
            hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp), TEXT("Play "));
            break;
        case evxShowInfo:
            g_ss.nXHelp = 70;
            g_ss.nYHelp = yPos;
            hr = StringCchCopy(g_ss.achHelp, NUMELMS(g_ss.achHelp),  TEXT("Show statistics "));
            break;
    }// switch
}
    ...
    )
{
    ...
    switch(lEventCode)
    {
        ...
        case EC_COMPLETE:
            ...
            pMovie->SeekToPosition(0L,FALSE);
            break;
