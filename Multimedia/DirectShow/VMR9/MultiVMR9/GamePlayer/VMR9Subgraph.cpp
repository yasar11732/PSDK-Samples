...
*
\**************************************************************************/
void CVMR9Subgraph::GetPathT( TCHAR* achPath, size_t len )
{
    if( achPath )
    {
        HRESULT hr = StringCchCopy( achPath, len, m_achPath);
    }
}

void CVMR9Subgraph::GetPathW( WCHAR* wcPath, size_t len )
{
    if( wcPath )
    {
        HRESULT hr = StringCchCopyW( wcPath, len, m_wcPath);
    }
}
