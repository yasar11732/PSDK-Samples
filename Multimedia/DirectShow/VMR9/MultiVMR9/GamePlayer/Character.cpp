        ...
        if (NumMaterials > 0)
        {
            ...
            for (iMaterial = 0; iMaterial < NumMaterials; iMaterial++)
            {
                if (pMeshContainer->pMaterials[iMaterial].pTextureFilename != NULL)
                {
                    ...
                    TCHAR strTextureFileName[MAX_PATH] = _T("");

                    (void)StringCchCopy( strTextureFileName, MAX_PATH , TEXT("Tiny\\"));
                    (void)StringCchCat( strTextureFileName, MAX_PATH , pMeshContainer->pMaterials[iMaterial].pTextureFilename);

                    hr = FindMediaFile(  strTexturePath, MAX_PATH, strTextureFileName, NULL, NULL );
                    if( SUCCEEDED( hr ))
                    {
                        hr = D3DXCreateTextureFromFile( pd3dDevice, strTexturePath, &pMeshContainer->ppTextures[iMaterial] );
                    }
                    if( FAILED( hr ) )
                    {
