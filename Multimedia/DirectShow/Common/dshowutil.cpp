...
HRESULT GetFileDurationString(IMediaSeeking *pMS, TCHAR *szDuration, size_t len)
{
    HRESULT hr;
    ...
    hr = StringCchCopy(szDuration, len, TEXT("<00:00.000>\0"));
    if (S_OK != hr)
    {
        return hr;
    }
    ...
    // Update the string
    hr = StringCchPrintf(szDuration, len, _T("%02dm:%02d.%03ds\0"), nMinutes, nSeconds, nMS);
