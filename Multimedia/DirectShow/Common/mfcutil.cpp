...
//
// The version of atlbase.h which shipped with Visual Studio .NET 
...
BOOL GetFileInfoStrings(LPTSTR szFile, TCHAR *szSize, size_t lenSize, TCHAR *szDate, size_t lenDate)
{
    HANDLE hFile;
    LONGLONG llSize=0;
    ...
    if (szSize)
    {
        ...
        // Update size string
        (void)StringCchPrintf(szSize, lenSize, TEXT("Size: %d bytes\0"), dwSizeLow);
    }
    ...
    if (szDate)
    {
        ...
        if (GetFileInformationByHandle(hFile, &fi))
        {
            ...
            // Update date string
            (void)StringCchPrintf(szDate, lenDate, TEXT("File date: %02d/%02d/%d\0"), 
