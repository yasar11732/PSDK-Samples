...
void GetGUIDString(TCHAR *szString, size_t len, GUID *pGUID)
{
    int i=0;
    HRESULT hr;
    ...
    while (rgng[i].pguid != 0)
    {
        ...
        {
            hr = StringCchPrintf(szString, len, TEXT("%s\0"), rgng[i].psz);
            return;
        }
        i++;
    }
    ...
    hr = StringCchCopy(szString, len, TEXT("GUID_NULL\0"));
}
...
void GetFormatString(TCHAR *szFormat, DMO_MEDIA_TYPE *pType, int nFormatSize)
{
    ...
    while (rgng[i].pguid != 0)
    {
        if(pType->formattype == *(rgng[i].pguid))
        {
            ...
            szFormat[nFormatSize-1] = TEXT('\0');   // Ensure null termination
            return;
        }
        i++;
    }
    ...
    // If we got here, there was no match
    (void)StringCchCopy(szFormat, nFormatSize,  TEXT("Format_None\0"));
}


void GetTypeSubtypeString(TCHAR *szCLSID, size_t len, DMO_PARTIAL_MEDIATYPE& aList)
{
    HRESULT hr;
