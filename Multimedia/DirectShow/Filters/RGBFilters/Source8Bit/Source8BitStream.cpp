...
//**************************************************************************
//
CSource8BitStream::CSource8BitStream(HRESULT *phr, CSource8Bit *pParent, LPCWSTR pPinName)
     : CSourceStream(NAME("Source8Bit"),phr, pParent, pPinName)
     , CSourceSeeking( NAME("Source8Bit"), (IPin*) this, phr, &m_SeekLock )
     , m_iDelivered( 0 )
     , m_nX( 0 )
     , m_nY( 0 )
     , m_nVx( 6 )
...
//**************************************************************************
//
CSource8BitStream::~CSource8BitStream()
{
}
...
//**************************************************************************
//
STDMETHODIMP CSource8BitStream::NonDelegatingQueryInterface( REFIID riid, void ** ppv )
{
...
//**************************************************************************
//
HRESULT CSource8BitStream::ChangeRate( )
{
    return NOERROR;
}
...
//**************************************************************************
//
HRESULT CSource8BitStream::GetMediaType(int iPosition, CMediaType *pmt)
{
...
//**************************************************************************
//
HRESULT CSource8BitStream::CheckMediaType(const CMediaType *pMediaType)
{
...
//**************************************************************************
//
HRESULT CSource8BitStream::DecideBufferSize(IMemAllocator *pAlloc,
...
//**************************************************************************
//
HRESULT CSource8BitStream::OnThreadStartPlay( )
{
...
//**************************************************************************
//
HRESULT CSource8BitStream::FillBuffer(IMediaSample *pms)
{
    ...
    memset( pData, counter++, DEFAULT_HEIGHT * DEFAULT_WIDTH );

    for( int x = m_nX ; x < m_nX + m_nSize ; x++ )
    {
        for( int y = m_nY ; y < m_nY + m_nSize ; y++ )
        {
            *( pData + y * DEFAULT_WIDTH + x ) = (BYTE) (x + 255 * cos( ( y + counter ) / 64.0 ));
        }
    }
...
//**************************************************************************
//
HRESULT CSource8BitStream::ChangeStart( )
{
    m_iDelivered = 0;
...
//**************************************************************************
//
HRESULT CSource8BitStream::ChangeStop( )
{
    return NOERROR;
}
