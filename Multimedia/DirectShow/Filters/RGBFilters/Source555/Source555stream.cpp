...
//**************************************************************************
//
CSource555BitStream::CSource555BitStream(HRESULT *phr, CSource555Bit *pParent, LPCWSTR pPinName)
     : CSourceStream(NAME("Source555Bit"),phr, pParent, pPinName)
     , CSourceSeeking( NAME("Source555Bit"), (IPin*) this, phr, &m_SeekLock )
     , m_iDelivered( 0 )
     , m_nX( 0 )
     , m_nY( 0 )
     , m_nVx( 6 )
...
//**************************************************************************
//
CSource555BitStream::~CSource555BitStream()
{
}
...
//**************************************************************************
//
STDMETHODIMP CSource555BitStream::NonDelegatingQueryInterface( REFIID riid, void ** ppv )
{
...
//**************************************************************************
//
HRESULT CSource555BitStream::ChangeRate( )
{
    return NOERROR;
}
...
//**************************************************************************
//
HRESULT CSource555BitStream::GetMediaType(int iPosition, CMediaType *pmt)
{
...
//**************************************************************************
//
HRESULT CSource555BitStream::CheckMediaType(const CMediaType *pMediaType)
{
...
//**************************************************************************
//
HRESULT CSource555BitStream::DecideBufferSize(IMemAllocator *pAlloc,
...
//**************************************************************************
//
HRESULT CSource555BitStream::OnThreadStartPlay( )
{
...
//**************************************************************************
//
HRESULT CSource555BitStream::FillBuffer(IMediaSample *pms)
{
    ...
    memset( pData, counter++, DEFAULT_HEIGHT * DEFAULT_WIDTH * 2 );

    for( int x = m_nX ; x < m_nX + m_nSize ; x++ )
    {
        for( int y = m_nY ; y < m_nY + m_nSize ; y++ )
        {
            *( pData + y * DEFAULT_WIDTH + x ) = (short) (x + 255 * cos( ( y + counter ) / 64.0 ));
        }
    }
...
//**************************************************************************
//
HRESULT CSource555BitStream::ChangeStart( )
{
    m_iDelivered = 0;
...
//**************************************************************************
//
HRESULT CSource555BitStream::ChangeStop( )
{
    return NOERROR;
}
