...
#define DEFAULT_FILENAME    TEXT("c:\\\0")
...
HRESULT CPlayDMODlg::AddDMOsToGraph(void)
{
    ...
    for (int i=0; i < nSelected; i++)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            (void)StringCchCopyW(wszDMOName, NUMELMS(wszDMOName), T2W(szItem));
...
HRESULT CPlayDMODlg::HandleGraphEvent(void)
{
    ...
    while(SUCCEEDED(hr = m_pME->GetEvent(&evCode, &evParam1, &evParam2, 0)))
    {
...
void CPlayDMODlg::ShowInputBufferInfo(IMediaObject *pDMO, int nSel)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = StringCchPrintf(sz, NUMELMS(sz), TEXT("%ld"), dwLatency);
    ...
    if (SUCCEEDED(hr))
    {
        hr = StringCchPrintf(sz, NUMELMS(sz), TEXT("%ld\0"), dwSize);
        m_nInBufferSize.SetWindowText(sz);

        hr = StringCchPrintf(sz, NUMELMS(sz), TEXT("%ld\0"), dwLookahead);
        m_nInLookahead.SetWindowText(sz);

        hr = StringCchPrintf(sz, NUMELMS(sz), TEXT("%ld\0"), dwAlign);
...
HRESULT CPlayDMODlg::ConnectDMOsToRenderer()
{
    ...
    (void)StringCchCopyW(wszFilter, NUMELMS(wszFilter), T2W(TEXT("Default DirectSound Device")));
