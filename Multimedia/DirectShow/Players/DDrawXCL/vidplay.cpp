...
// playback.
//
HRESULT CDVDPlayer::BuildGraph(HWND hWndApp, LPDIRECTDRAW pDDObj, 
                               LPDIRECTDRAWSURFACE pDDPrimary)
{
    ...
    {
#ifdef UNICODE
        (void)StringCchCopy(achwFileName, NUMELMS(achwFileName), GetFileName()) ;
#else
        MultiByteToWideChar(CP_ACP, 0, GetFileName(), -1, achwFileName, NUMELMS(achwFileName)) ;
#endif // UNICODE
    ...
    if (S_FALSE == hr)  // if partial success
    {
        ...
        {
            (void)StringCchCopy(achBuffer, NUMELMS(achBuffer),TEXT("An unknown error has occurred")) ;
        }
        (void)StringCchCat(achBuffer, NUMELMS(achBuffer), TEXT("\n\nDo you still want to continue?")) ;
                                ...
                                DWORD dwMaxText)
{
    ...
    if (pStatus->iNumStreamsFailed > 0)
    {
        ...
        {
            (void)StringCchPrintf(lpszBuff, len,TEXT("    - audio stream\n")) ;
            iChars = lstrlen(lpszBuff);
            lpszBuff += iChars ;
            len -= iChars;
        }
        ...
        {
            (void)StringCchPrintf(lpszBuff, len, TEXT("    - subpicture stream\n")) ;
            iChars = lstrlen(lpszBuff);
            lpszBuff += iChars ;
            len -= iChars;
        }
    ...
    {
        (void)StringCchCat(lpszBuff,len,  TEXT("* ")) ;
        iChars = lstrlen(TEXT("* ")) ;
        lpszBuff += iChars;
        len -= iChars;
    ...
    {
        (void)StringCchPrintf(lpszBuff, len, TEXT("* Specified DVD-Video volume was invalid\n")) ;
        iChars = lstrlen(lpszBuff);
        lpszBuff += iChars ;
        len -= iChars;
    }
    ...
    {
        (void)StringCchPrintf(lpszBuff, len, TEXT("* No valid DVD-Video volume could be located\n")) ;
        iChars = lstrlen(lpszBuff);
        lpszBuff += iChars ;
        len -= iChars;
    }
    ...
    {
        (void)StringCchPrintf(lpszBuff, len, TEXT("* The video decoder doesn't produce closed caption data\n")) ;
        iChars = lstrlen(lpszBuff);
        lpszBuff += iChars ;
        len -= iChars;
    }
    ...
    {
        (void)StringCchPrintf(lpszBuff, len, TEXT("* Decoded closed caption data not rendered properly\n")) ;
        iChars = lstrlen(lpszBuff);
        lpszBuff += iChars ;
        len -= iChars;
    }
    
    DWORD dwLength = (DWORD) ((lpszBuff - achBuffer) * sizeof(*lpszBuff)) ;
    dwLength = min(dwLength, dwMaxText) ;
    (void)StringCchCopy(lpszStatusText, dwLength, achBuffer) ;
...
HRESULT CFilePlayer::BuildGraph(HWND hWndApp, LPDIRECTDRAW pDDObj, LPDIRECTDRAWSURFACE pDDPrimary)
{
    ...
    {
#ifdef UNICODE
        lstrcpyn(achwFileName, GetFileName(), MAX_PATH) ;
#else
        MultiByteToWideChar(CP_ACP, 0, GetFileName(), -1, achwFileName, NUMELMS(achwFileName)-1) ;
#endif // UNICODE
        achwFileName[MAX_PATH-1] = TEXT('\0');  // Ensure NULL termination
...
//
HRESULT CFilePlayer::SetVideoPosition(DWORD dwLeft, DWORD dwTop, 
...
//
HRESULT CFilePlayer::GetNativeVideoData(DWORD *pdwWidth, DWORD *pdwHeight, 
                                        DWORD *pdwARX, DWORD *pdwARY)
{
