...
CDDrawObject::CDDrawObject(HWND hWndApp)
{
    ...
    m_iCount = 0 ;
    m_bFrontBuff = TRUE ;
    m_szFrontMsg = TEXT("Front Buffer\0" );
    m_szBackMsg  = TEXT("Back Buffer\0" );
    m_szDirection = TEXT("Press the 'Arrow' keys to move the ball. Hit 'Esc' to stop playback.\0" );
...
void CDDrawObject::DrawOnSurface(LPDIRECTDRAWSURFACE pSurface)
{
    ...
    if (DD_OK == pSurface->GetDC(&hDC))
    {
        ...
        (void)StringCchPrintf(achBuffer, NUMELMS(achBuffer), TEXT(" %-12s: %6.6d "), 
