                     ...
                     int       nCmdShow)
{
    ...
    LoadString(ghInstance, IDS_APP_TITLE, gszAppTitle, NUMELMS(gszAppTitle)) ;
    LoadString(ghInstance, IDS_APP_NAME,  gszAppName,  NUMELMS(gszAppName)) ;
...
BOOL FileSelect(HWND hWnd, VIDEO_TYPE eType)
{
    ...
    if (DVD == eType)
        (void)StringCchCopy(achFileName, NUMELMS(achFileName), TEXT("*.ifo\0")) ;
    else
        (void)StringCchCopy(achFileName, NUMELMS(achFileName), TEXT("*.avi\0")) ;
    ...
    if (DVD == eType)
    {
        ...
        ofn.lpstrFilter = TEXT("IFO Files (*.ifo)\0*.ifo\0All Files (*.*)\0*.*\0\0") ;
    }
    else
    {
        ofn.lpstrTitle = TEXT("Select Video file\0") ;
        ofn.lpstrFilter = TEXT("AVI Files (*.avi)\0*.avi\0MPEG Files (*.mpg)\0*.mpg\0All Files(*.*)\0*.*\0\0") ;
    }
...
void SetVideoPosition(BOOL bSetBallPosition)
{
    ...
    gpDDrawObj->SetVideoPosition(dwVideoLeft, dwVideoTop, 
                                 RECTWIDTH(rectCorrected), 
                                 RECTHEIGHT(rectCorrected)) ;
...
LRESULT StartPlay(HWND hWndApp)
{
    ...
    //
    // Make sure that DirectShow components are installed etc so that
    // the graph building can be init-ed
    //
    if (! gpPlayer->Initialize() )
    {
        MessageBox(hWndApp, 
            TEXT("DirectShow components couldn't be initialized.\n\nCan't run test.\nPlease check DxMedia installation"), 
