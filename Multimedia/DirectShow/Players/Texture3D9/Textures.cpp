...
VOID DrawText()
{
    ...
    if( g_pRenderer )
    {
        ...
        if( SUCCEEDED(hr))
        {
            ...
            if( SUCCEEDED(hr))
            {
                (void)StringCchPrintf( achMsg, NUMELMS(achMsg), TEXT("FPS = %5.3f"), (float)nFPS / 100.f);
            }
    ...
    {
        (void)StringCchPrintf( achMsg, NUMELMS(achMsg), TEXT("Rendering method: %s"), g_pachRenderMethod);
        g_pFont->DrawText(5, 40, D3DCOLOR_ARGB(0xCC,0xFF,0xFF,0x00), achMsg );
    }

    (void)StringCchPrintf( achMsg, NUMELMS(achMsg), TEXT("[Space]: change background color"));
    g_pFont->DrawText(5, 60, D3DCOLOR_ARGB(0xCC,0xFF,0xFF,0x00), achMsg );
    (void)StringCchPrintf( achMsg, NUMELMS(achMsg), TEXT("[Enter]: set background color to black"));
