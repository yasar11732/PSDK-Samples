...
//Handle the command line arguments.
    ...
    wcsncpy( pszBuffer, argv[1], MAX_PATH * 2 ) ;
...
//Get rootDSE and the domain container's DN.
    ...
    if ( SUCCEEDED(hr) )
    {
        _snwprintf( szPath, MAX_PATH, L"LDAP://%ws", var.bstrVal ) ;
        szPath[MAX_PATH-1] = 0 ;    
        VariantClear(&var);
        if ( pObject )
        {
                        ...
                        IADs **ppUser )
{
    ...
    if ( 0 > _snwprintf( pszSearchFilter, MAX_PATH, L"(&(objectCategory=person)(objectClass=user)(cn=%ws))", szFindUser ) ) {
    ...
    if ( SUCCEEDED( hr ) ) {
        if ( S_ADS_NOMORE_ROWS != pSearchBase->GetNextRow( hSearch ) ) {       
            ...
            if ( SUCCEEDED( hr ) ) {
                wcsncpy( pszADsPath, col.pADsValues->CaseIgnoreString, MAX_PATH ) ;
