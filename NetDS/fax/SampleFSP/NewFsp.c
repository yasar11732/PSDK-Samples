...
Routine Description:
  ...
  The fax service calls the FaxDevInitialize function each time the service starts to initialize communication 
  between the fax service and the fax service provider DLL
...
Routine Description:
  ...
  The fax service calls the FaxDevSend function to signal a fax service provider that it must initiate an outgoing 
  fax transmission
...
--*/
{
	...
	lstrcpy(szDeviceFolder, g_pDeviceInfo[iDestinationDevice]->Directory);
	...
	WriteDebugString(L"   Dest. device Folder     : 0x%08x\n", szDeviceFolder);
    ...
    lstrcpy(szDestinationFilename, szDeviceFolder);
    lstrcat(szDestinationFilename, L"\\");
    lstrcat(szDestinationFilename, szSourceName);
    lstrcat(szDestinationFilename, L".tif");
...
--*/
{
    ...
    lstrcat(szCurrentFilename, L"\\SampleFSP.dll");
	...
	wcscat(szWinFaxDllLocation,L"\\winfax.dll" );
