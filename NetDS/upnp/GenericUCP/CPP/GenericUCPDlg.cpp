...
void CGenericUCPDlg::ProcessFindByUDN(BSTR bstrSearchType)
{
	...
	if(hr==S_OK){
		...
		if(SUCCEEDED(hr)){
			TCHAR tszFriendlyName[DATA_BUFSIZE];
			_sntprintf(tszFriendlyName, DATA_BUFSIZE-1, _T("%S"), bstrFriendlyName);
			m_DeviceCombo.AddString(tszFriendlyName);
    ...
    for (int i=0; i < nCount; i++)
    {
        ...
        if(SUCCEEDED(hr))
        {
            if(wcscmp(bstrTemp, bstrUDN)==0)
            {
                ...
                ReleaseObj(pDevice); //Since we are going to remove the device, we have to release it totally.
                m_DeviceCombo.DeleteString(i);
                i--; // So that we search the next device in the list
                nCount--;
                ...
                if(bstrActionName!=NULL)
                {
                         ...
                         FAILED(HrCreateArgVariants(iArgs, &ppVars))))
                    {
                        ...
                        while(token!=NULL)
                        {
                            ...
                            token = _tcstok(NULL, _T(" ")); // We have the space character as the separator
                            iCount++;
                        }
	...
	VARIANT varValue)
{
	...
	if(SUCCEEDED(hr)){
		...
		if(SUCCEEDED(hr)){
			_sntprintf(tszMessage, DATA_BUFSIZE-1,_T("State variable %S changed to %S in %S"),pcwszStateVarName, varValue.bstrVal, bstrServiceId);
			Sleep(350);
	...
	IUPnPService *pus)
{
	...
	hr = pus->get_Id(&bstrServiceId);
	if(SUCCEEDED(hr)){
		_sntprintf(tszMessage, DATA_BUFSIZE-1, _T("Service %S died"),bstrServiceId);
		m_pGenericUCPDlg->m_EventText.SetWindowText(tszMessage);
		SysFreeString(bstrServiceId);
	}
	else{
