...
void SafeStrCopy(TCHAR* src, TCHAR* dst, size_t srcBufLen)
{
   ...
   _tcsncpy(src, dst, (srcBufLen - 1));

   // Put a NULL terminator at the end of the buffer. tcsncpy will not NULL
   // terminate the string if the dst length is greater than the size 
   // specified.
   src[srcBufLen - 1] = NULL;
