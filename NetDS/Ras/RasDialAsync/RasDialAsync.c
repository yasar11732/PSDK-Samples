...
int main(int argc, char*argv[])
{
    ...
    _snprintf(szTempBuf, CELEMS(szTempBuf) - 1, "Dialing... %s\n", lpRasDialParams->szPhoneNumber);
    printf(szTempBuf);
    hRasConn = NULL;
    ...
    {
        ZeroMemory((LPVOID)szTempBuf, sizeof(szTempBuf));  
        _snprintf(szTempBuf, CELEMS(szTempBuf) - 1, "Connection to %s terminated.\n", lpRasDialParams->szPhoneNumber);
        printf(szTempBuf);
    }
    else
    {
                        ...
                        DWORD dwError )
{
    ...
    if (dwError)  // Error occurred
    {
        ...
        ZeroMemory((LPVOID)szTempBuf, sizeof(szTempBuf));  
        _snprintf(szTempBuf, CELEMS(szTempBuf) - 1, "Error: %d - %s\n", dwError, szRasString);
        printf(szTempBuf);
