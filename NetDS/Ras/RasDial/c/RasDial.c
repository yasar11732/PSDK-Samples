...
int main(int argc, char **argv) 
{
            ...
            if (argv[i] && ((argv[i][0] == '-') || (argv[i][0] == '/')))
	        {
                switch(tolower(argv[i][1])) 
	            {
                    case 'e': // Entry name
                        ...
                        if (argv[j])
                        {
                            lstrcpyn(lpRasDialParams->szEntryName, argv[j], CELEMS(lpRasDialParams->szEntryName));
                            fRequired = TRUE;
                        }
                        break;
                    case 'p': // Phone number
                        ...
                        if (argv[j])
                        {
                            lstrcpyn(lpRasDialParams->szPhoneNumber, argv[j], CELEMS(lpRasDialParams->szPhoneNumber));
                        }
                        break;
                    case 'u': // User name
                        j = ++i;
                        if (argv[j])
                        {
                            lstrcpyn(lpRasDialParams->szUserName, argv[j], CELEMS(lpRasDialParams->szUserName));
                        }
                        break;
                    case 'z': // Password
                        j = ++i;
                        if (argv[j])
                        {
                            lstrcpyn(lpRasDialParams->szPassword, argv[j], CELEMS(lpRasDialParams->szPassword));
                        }
                        break;
                    case 'd': // Domain name
                        j = ++i;
                        if (argv[j])
                        {
                            lstrcpyn(lpRasDialParams->szDomain, argv[j], CELEMS(lpRasDialParams->szDomain));
                        }
                        break;
                    default:
                        Usage(argv[0]);
    ...
    else
    {
        ...
        if (lpRasConnStatus->rasconnstate == RASCS_Connected)
        {
            ...
            _snprintf(szTempBuf, CELEMS(szTempBuf) - 1, "Connection estabilished using %s\n", lpRasConnStatus->szDeviceName);
            printf(szTempBuf);
        }
        else
        {
   ...
   default:
    ...
    if (fSuccess)
    {
        ...
        ZeroMemory((LPVOID)szTempBuf, sizeof(szTempBuf));
        _snprintf(szTempBuf, CELEMS(szTempBuf) - 1, "\nRas Client IP address: %s\n", lpProjection->szIpAddress);
        printf(szTempBuf);

        ZeroMemory((LPVOID)szTempBuf, sizeof(szTempBuf));  
        _snprintf(szTempBuf, CELEMS(szTempBuf) - 1, "Ras Server IP address: %s\n\n", lpProjection->szServerIpAddress);
        printf(szTempBuf);
    }
