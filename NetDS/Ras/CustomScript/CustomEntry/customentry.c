...
int __cdecl main (int argc, TCHAR * argv[])
{
    ...
    if (3 <= argc)
    {
        if (3 == argc)
        {
            lstrcpyn(szPhoneNumber, argv[1], CELEMS(szPhoneNumber));
        ...
        else
        {
            if (4 == argc)
            {
                lstrcpyn(szPhoneNumber, argv[1], CELEMS(szPhoneNumber));
                fCustomScript = (BOOL)_ttol(argv[2]);
                lstrcpyn(szCustomDLLPath, argv[3], CELEMS(szCustomDLLPath));   
            }
            else
            {
                Usage();
    ...
    lstrcpyn(szEntryName, RASCUSTOMDIAL_ENTRYNAME, CELEMS(szEntryName));
...
BOOL CreateEntry(LPTSTR pszEntryName, LPTSTR pszPhoneNumber, LPTSTR pszDLLPath, BOOL fCustomScript)
{
    ...
    lstrcpyn(pRasEntry->szLocalPhoneNumber, pszPhoneNumber, CELEMS(pRasEntry->szLocalPhoneNumber));
    lstrcpyn(pRasEntry->szDeviceType, RASDT_Modem, CELEMS(pRasEntry->szDeviceType));
    lstrcpyn(pRasEntry->szCustomDialDll, pszDLLPath, CELEMS(pRasEntry->szCustomDialDll));
    ...
    if (fSuccess)
    {
        ...
        {           
            ...
            {    
                ...
                lstrcpyn(pRasEntry->szDeviceName, lpRasDevInfoTemp->szDeviceName, CELEMS(pRasEntry->szDeviceName));   
                break;
            }            
        }
    }
...
void OutputTraceString(LPTSTR lpszFormatString, ...)
{
  ...
  _vsnprintf(szOutputString, MAX_DEBUGSTR, lpszFormatString, arglist);
  va_end(arglist);
