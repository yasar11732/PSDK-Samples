...
VOID GetEEInfoText(TCHAR *Msg, UINT BuffSize, UINT *MsgSize) {
	...
	*MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("EEInfo:\n"));

    status = RpcErrorStartEnumeration(&EnumHandle);

    if (status == RPC_S_ENTRY_NOT_FOUND) {
        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("no EEInfo found\n"));
    }
    else if (status != RPC_S_OK) {
        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("could not get EEInfo\n"));
    }
    else {
        ...
        while (status == RPC_S_OK) {
            ...
            if (status == RPC_S_ENTRY_NOT_FOUND) {
                ...
                *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("enumeration broke with status %d\n"), status);
                break;
            }
            else {
                INT i;
                
                *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("GeneratingComponent: %d\n"), ErrorInfo.GeneratingComponent);
                *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("Status: %d\n"), ErrorInfo.Status);
                *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("Detection Location: %d\n"), ErrorInfo.DetectionLocation);
                *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("Flags: %d\n"), ErrorInfo.Flags);
                *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("NumberOfParameters: %d\n"), ErrorInfo.NumberOfParameters);
                
                for (i = 0; i < ErrorInfo.NumberOfParameters; i++) {
                    switch (ErrorInfo.Parameters[i].ParameterType) {
                    case eeptAnsiString:
                        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("AnsiString: %s\n"), ErrorInfo.Parameters[i].u.AnsiString);
                        break;
                        
                    case eeptUnicodeString:
                        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("UnicodeString: %S\n"), ErrorInfo.Parameters[i].u.UnicodeString);                        
                        break;
                        
                    case eeptLongVal:
                        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("LongVal: %d\n"), ErrorInfo.Parameters[i].u.LVal);
                        break;
                        
                    case eeptShortVal:
                        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("ShortVal: %d\n"), ErrorInfo.Parameters[i].u.SVal);                        
                        break;
                        
                    case eeptPointerVal:
                        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("PointerVal: %d\n"), ErrorInfo.Parameters[i].u.PVal);                        
                        break;
                        
                    case eeptNone:
                        *MsgSize += _sntprintf(&Msg[*MsgSize], BuffSize - *MsgSize, TEXT("Invalid type of parameter %d\n"), ErrorInfo.Parameters[i].ParameterType);
                        break;
