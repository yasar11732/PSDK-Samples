...
VOID ReadData(HANDLE hFile, VOID *pbBuf, unsigned long nToRead, unsigned long *cbRead, BOOL *bEOF, BYTE *EOFCheck) {
    ...
    if(!ReadFile(hFile, pbBuf, nToRead+1, cbRead, NULL)) {
        ...
        AddRpcEEInfoAndRaiseException(GetLastError(), TEXT("ReadData: ReadFile failed"));
        return;
    }
                  ...
                  unsigned long *cbRead)
{
    ...
    // The priority level of the calling user.
    UINT Pri;

    // Set if we reached the maximum number of concurrent requests.
    BOOL bMaxConcurrentRequests = FALSE;
    ...
    if (!bImpersonating) {
        if ((rpcstatus = RpcImpersonateClient(hFileRepServer)) != RPC_S_OK) {      
            AddRpcEEInfoAndRaiseException(rpcstatus, TEXT("s_RemoteOpen: RpcImpersonateClient failed"));
            return;
        }
        bImpersonating = TRUE;
    }
    ...
    Pri = GetCurrentUserPriority();

    // Increment the counter for the number of concurrent requests.
    if (!CounterIncrement(pServerReqCounters[Pri])) {
        AddRpcEEInfoAndRaiseException(RPC_S_SERVER_TOO_BUSY, TEXT("s_RemoteOpen: CounterIncrement failed"));
        return;
    }
    ...
    if ((hFile = CreateFile(pszFileName,
        ...
        if (bImpersonating) {
            if ((rpcstatus = RpcRevertToSelf()) != RPC_S_OK) {
                CounterDecrement(pServerReqCounters[Pri]);
                AddRpcEEInfoAndRaiseException(rpcstatus, TEXT("s_RemoteOpen: RpcRevertToSelf failed"));
        ...
        CounterDecrement(pServerReqCounters[Pri]);
        AddRpcEEInfoAndRaiseException(GetLastError(), TEXT("s_RemoteOpen: CreateFile failed"));
    ...
    if(bEOF) {
        ...
        *pphContext = (PCONTEXT_HANDLE_TYPE) pFileContext;
        ...
        if((pFileContext = (FILE_CONTEXT_TYPE *) midl_user_allocate(sizeof(FILE_CONTEXT_TYPE))) == NULL) {
            ...
            CounterDecrement(pServerReqCounters[Pri]);
            AddRpcEEInfoAndRaiseException(ERROR_OUTOFMEMORY, TEXT("s_RemoteOpen: midl_user_allocate failed"));
                  ...
                  unsigned long *cbRead)
{
    ...
    // We need to impersonate the user that has issued the
    // remote call.  We need to do this to determine
    // the priority level of the requests below.
    if (!bImpersonating) {
        if ((rpcstatus = RpcImpersonateClient(hFileRepServer)) != RPC_S_OK) {      
            AddRpcEEInfoAndRaiseException(rpcstatus, TEXT("s_RemoteRead: RpcImpersonateClient failed"));
            return;
        }
        bImpersonating = TRUE;
    }
    ...
    if (bImpersonating) {
        if ((rpcstatus = RpcRevertToSelf()) != RPC_S_OK) {
            AddRpcEEInfoAndRaiseException(rpcstatus, TEXT("s_RemoteRead: RpcRevertToSelf failed"));
    ...
    // Increment the counter for the number of concurrent requests.
    if (!CounterIncrement(pServerReqCounters[Pri])) {
        AddRpcEEInfoAndRaiseException(RPC_S_SERVER_TOO_BUSY, TEXT("s_RemoteRead: CounterIncrement failed"));
        return;
    }
    ...
    if(*pphContext == NULL) {
        ...
        AddRpcEEInfoAndRaiseException(RPC_X_SS_IN_NULL_CONTEXT, TEXT("s_RemoteRead: pphContext == NULL"));
