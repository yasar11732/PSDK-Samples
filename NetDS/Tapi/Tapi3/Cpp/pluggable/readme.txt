...
Before building the PLGTERM sample application you need to 
	...
	-build the $(DXSDKROOT)\Samples\Multimedia\DirectShow\BaseClasses sample
	to produce UNICODE strmbase.lib or UNICODE strmbasd.lib file before building PlgTerm sample 
		...
		using Visdual Studio's build configuration manager to build UNICODE libraries
	-run the setenv.bat file in the Platform SDK root dir
	-set the DXSDKROOT enviroment variable to point to the DirectX SDK install root. You can do this in the setenv.bat in the
	Microsoft Platform SDK root directory
...
-	build the $(DXSDKROOT)\Samples\C++\DirectShow\BaseClasses 
