    ...
    )
{
    ...
    if (lpLineDevCaps->dwTotalSize >= lpLineDevCaps->dwNeededSize)
    {
        ...
        strncpy (szLineConfig, gszDefLineConfigParams, LINECONFIG_SIZE-1);
		szLineConfig[LINECONFIG_SIZE-1] = '\0';
    ...
    )
{
    ...
    case WM_INITDIALOG:
    {
        ...
        for (i = 0; i < iNumLines; i++)
        {
	     ...
	     if (!(pLineConfig = DrvAlloc (sizeof(DRVLINECONFIG))))
            ...
            if (0 != lResult)
            {
               strncpy (pBuf, gszDefLineConfigParams, 255);
				pBuf[255] = '\0';
            }
            ...
            *p2 = 0;
            ...
            strncpy (pLineConfig->szPort, p, 7 );
			pLineConfig->szPort[7] = '\0';

            p = p2 + 1;

            strncpy (pLineConfig->szCommands, p,63);
			pLineConfig->szCommands[63] = '\0';
        }
    ...
    case WM_COMMAND:
    {
        ...
        case IDC_ADD:
        {
            ...
            strncpy (pLineConfig->szPort, "COM1", 7);
			pLineConfig->szPort[7] = '\0';

            strncpy (szLineName, "my new line",31 );
			szLineName[31] = '\0';
    ...
    )
{
    if (dwDbgLevel <= gdwDebugLevel)
    {
        ...
        _vsnwprintf(&buf[8], 119, lpszFormat, ap );
