...
VOID ReadINI( VOID ) 
{
    ...
    for(cSDEntry = 1; cSDEntry <= NSPEEDDIALS; ++cSDEntry)
    {
		...
		{
			strncpy( szName, gszSDNumber[ cSDEntry ], (sizeof(szName)-1));
		}
		
		szName[TAPIMAXCALLEDPARTYSIZE-1] = '\0';
                ...
                switch( LOWORD( (DWORD)wParam ) )
                {
                    ...
                    case IDM_LOCATION:
                    {
                           ...
                           )
                        {
                            ...
                            {
                                strncpy( szCanNumber, szNumber,(sizeof(szCanNumber)-1)); 
								szCanNumber[TAPIMAXDESTADDRESSSIZE-1] = '\0';
							}
                        }
...
VOID InitiateCall ( LPCSTR szNumber, LPCSTR szName )
{
	...
	do
	{
		...
		if ( !MakeCanonicalNumber( szNumber, szCanNumber ) )
		{
			strncpy( szCanNumber, szNumber, TAPIMAXDESTADDRESSSIZE -1 );
			szCanNumber[TAPIMAXDESTADDRESSSIZE-1] = '\0';
		}
	...
	if ( szName[ 0 ] )
	{
		...
		((LPSTR)lpLineCallParams)[lpLineCallParams->dwTotalSize-1] = '\0';
	...
	strncpy ( gszCurrentName, szName, dwNameLen );
	gszCurrentName[dwNameLen-1] = '\0';
	...
	strncpy ( 
			 gszCurrentNumber, 
			 (LPSTR) lpTransOut + lpTransOut-> dwDisplayableStringOffset ,
			lpTransOut-> dwDisplayableStringSize-1
			);

	gszCurrentNumber[lpTransOut-> dwDisplayableStringSize-1] = '\0';
...
DWORD GetLineInfo ( DWORD iLine, LPLINEINFO lpLineInfo )
{
	...
	strncpy( lpLineInfo->szLineName, pszLineName,MAXBUFSIZE - 1 );
	lpLineInfo->szLineName[ MAXBUFSIZE - 1 ] = '\0';
	...
	switch(msg)
	{
		case WM_INITDIALOG:
		{
			...
			for(cSDEntry = 1; cSDEntry <= NSPEEDDIALS; ++cSDEntry)
			{
				...
				{			
					strncpy( szName, szSDName[ cSDEntry], TAPIMAXCALLEDPARTYSIZE-1 );
				}
				
				else
				{
					strncpy( szName, gszSDNumber[ cSDEntry ],TAPIMAXCALLEDPARTYSIZE-1 );
				}
				szName[TAPIMAXCALLEDPARTYSIZE-1] = '\0';
				...
				case IDD_SD1EDITNAME:
					if ( HIWORD( wParam ) == EN_CHANGE )
					{
						...
						if ( szName[ 0 ] == '\0' )
						{
							...
							strncpy( szName, gszSDNumber[ nCurrentSpeedDial ],TAPIMAXCALLEDPARTYSIZE-1 );
							szName[TAPIMAXCALLEDPARTYSIZE-1] = '\0';
						}
						else
						{
							strncpy( szSDName[ nCurrentSpeedDial ], szName,TAPIMAXDESTADDRESSSIZE-1 );
							szSDName[ nCurrentSpeedDial ][TAPIMAXDESTADDRESSSIZE-1] = '\0';
						}
...
VOID FitTextToButton ( HWND hDlg, INT nButtonID, LPSTR szName )
{
	...
	do
	{
		...
		// leading space,
		strncpy ( buf, szName, TAPIMAXCALLEDPARTYSIZE );
		buf[TAPIMAXCALLEDPARTYSIZE] = '\0';
		nLen = lstrlen( buf );
		buf [ nLen ] = 'X';
		buf [ nLen + 1 ] = '\0';
...
BOOL MakeCanonicalNumber ( LPCSTR szNumber, LPSTR szCanNumber )
{
	...
	switch ( nLenDigits )
	{
		...
		case LOCAL_NUMBER :
		{
			...
			szCanNumber[ 0 ]= '\0';
			strncat( szCanNumber, "+1 (",(sizeof(szCanNumber)-1));
			strncat( szCanNumber, (LPSTR) lpTransCaps + lpLocLst[ dwInd ].dwCityCodeOffset,
					sizeof(szCanNumber)- 5);
			strncat( szCanNumber, ") ",
					((sizeof(szCanNumber))-(5 + (lpTransCaps-> dwTotalSize) + (lpLocLst[ dwInd ].dwCityCodeSize)  )));
			strncat( szCanNumber, szDigits,
				((sizeof(szCanNumber))- (5 + (lpTransCaps-> dwTotalSize) + (lpLocLst[ dwInd ].dwCityCodeSize) + 2 )));


			cPos = (int) strcspn( szNumber, "|" );
			if ( cPos != lstrlen( szNumber ) )
			{
				strncat( szCanNumber, &(szNumber[ cPos ]), 
					((sizeof(szCanNumber) )- (5 + (lpTransCaps-> dwTotalSize) + (lpLocLst[ dwInd ].dwCityCodeSize) + 2 + 
					lstrlen(szDigits))));
			}
		...
		case LONG_DISTANCE_NUMBER:
		{
			...
			szCanNumber[ 0 ] = '\0';
			strncat( szCanNumber, "+1 (" , (sizeof(szCanNumber)-1));
			strncat( szCanNumber, szDigits, (sizeof(szCanNumber)-5));
			strncat( szCanNumber, ") ", (sizeof(szCanNumber)-5-lstrlen(szDigits)));

			strncat( szCanNumber, &(szDigits[ 3 ]),
				(sizeof(szCanNumber)-5-lstrlen(szDigits)-2));
