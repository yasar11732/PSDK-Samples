    ...
    )
{
    ...
    if (dwLastError >= NERR_BASE && dwLastError <= MAX_NERR)
    {
        ...
        TCHAR tcharPath[MAX_PATH+1];  // Space for the System32 path, the DLL, and the Null termination
        UINT  uiRet;                  // Length of the System32 Path returned by GetSystemDirectory

        // We know our DLL lives in the System32 directory, get the path and build the correct path
        // To make sure everything fits within our buffer, pass in the size of MAX_PATH minus the size of 
        // our dll string.
        uiRet = GetSystemDirectory(tcharPath, MAX_PATH - 10 ); 

        // Make sure the call succeeded 
        if ( (MAX_PATH - 10 > uiRet) && (0 != uiRet) ) 
        {
            // Concatenate the dll to our string
            lstrcat(tcharPath, TEXT("\\netmsg.dll") );

            // Call LoadLibraryEX with the complete path 
            hModule = LoadLibraryEx(
                                   tcharPath,       // the complete path to the netmsg.dll
                                   NULL,
                                   LOAD_LIBRARY_AS_DATAFILE
                                   );

        }
        // Did the LoadLibrary Succeed
        if (hModule != NULL)
