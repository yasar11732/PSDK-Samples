...
HRESULT MyOSPObject::Init(WCHAR* pwszFilePath)
{
	...
	wcstombs(m_szFilePath, pwszFilePath, FILENAME_MAX);
...
HRESULT MyOSPObject::loadData()
{
	...
	BSTR		pwszBuffer;		// -> wide character buffer
	BSTR		pwszRow;		// -> wide character buffer (position)
	BSTR		pwszToken;
	LPTSTR		pvInput;		// -> input buffer
	LPSTR		pszPos;
	LONG		i, j;

	// Build the internal array by reading the file:
	myOSPFile.open(m_szFilePath, ios::in | ios::nocreate, filebuf::sh_none );
...
HRESULT MyOSPObject::saveData()
{
	...
	myOSPFile.open(m_szFilePath, ios::out | ios::nocreate, filebuf::sh_none);
    if (!(myOSPFile.is_open()))
        return E_FAIL;
...
HRESULT MyOSPObject::getLocale(BSTR* pbstrLocale)
{
	...
	if (pszLocale)
	{
		...
		mbstowcs(bstr, pszLocale, cchData);
#else 
