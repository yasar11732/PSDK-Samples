...
)
{
	...
	for(DBCOUNTITEM i=0; i<cRows; i++)
		cstrList += StackFormat(L"%d%s", rghRows[i], (i+1<cRows) ? L"," : L"");
	cstrList += L"]"; 

	//Output Notification
	return TRACE_NOTIFICATION(NOTIFY_IROWSETNOTIFY, m_hrReturn, L"IRowsetNotify", L"OnRowChange", L"(%s, %s, 0x%p, %lu, %s, %s)", GetReasonName(eReason), GetPhaseName(ePhase), pIRowset, cRows, (WCHAR*)cstrList, fCantDeny ? L"TRUE" : L"FALSE");
}
