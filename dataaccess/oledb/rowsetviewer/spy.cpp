...
const ULONG HEADERSIZE = ROUNDUP(HEADSIZE + LENGTHSIZE + IDSIZE + FILENAMESIZE + LINENUMBERSIZE);
...
HRESULT CMallocSpy::DumpLeaks()
{
	...
	{	
		...
		if(HEAD_SIGNITURE(pActual) != HEADSIGN)
			InternalTraceFmt(L"TRACE - IMallocSpy HeadSigniture Corrupted! - 0x%p, ID=%08lu, %lu byte(s)\n", pRequest, BUFFER_ID(pActual), BUFFER_LENGTH(pActual));

		if(TAIL_SIGNITURE(pActual) != TAILSIGN)
			InternalTraceFmt(L"TRACE - IMallocSpy TailSigniture Corrupted! - 0x%p, ID=%08lu, %lu byte(s)\n", pRequest, BUFFER_ID(pActual), BUFFER_LENGTH(pActual));
		...
		if(dwSelection == IDOK)
		{
			dwSelection = wMessageBox(GetFocus(), MB_TASKMODAL | MB_ICONWARNING | MB_OKCANCEL | MB_DEFBUTTON1, 
				L"IMallocSpy Leak", 
					L"IMallocSpy Leak:\n"
					L"0x%p, ID=%08lu, %lu byte(s), File: %s, Line %d\n\n",
					pRequest, ulID, ulSize, pwszFileName, ulLine);
		}

		//Include FileName and Line Number of the leak...
		InternalTraceFmt(L"-- IMallocSpy Leak! - 0x%p,\tID=%08lu,\t%lu byte(s),\tFile: %s,\tLine %d" wWndEOL, pRequest, ulID, ulSize, pwszFileName, ulLine);
	...
	if(cTotalLeaks)
		InternalTraceFmt(L"-- IMallocSpy Total Leaks: %lu = %lu byte(s)" wWndEOL, cTotalLeaks, cTotalBytes);
	return S_OK;
}
...
void* CMallocSpy::PostAlloc(void* pActual)
{
	...
	if(GetErrorPosting(EP_IMALLOC_ALLOCS))
		InternalTraceFmt(L"TRACE - IMallocSpy Alloc - 0x%p,\tID=%08lu,\t%lu byte(s)\n", USERS_OFFSET(pActual), ulID, m_cbRequest);
...
void* CMallocSpy::PostRealloc(void* pActual, BOOL fSpyed)
{
    ...
    if(fSpyed)
    {
		...
		//Show ReAllocations
		if(GetErrorPosting(EP_IMALLOC_ALLOCS))
			InternalTraceFmt(L"TRACE - IMallocSpy Realloc - 0x%p,\tID=%08lu,\t%lu byte(s)\n", USERS_OFFSET(pActual), BUFFER_ID(pActual), m_cbRequest);
	...
	if(TRUE)
	{
		...
		switch(allocType)
		{
			case _HOOK_ALLOC:	// - only defined for DEBUG
				//Display the Alloc
				InternalTraceFmt(L"TRACE - CRT Alloc - 0x%p, ID=%08lu, %lu byte(s), File: %S, Line: %d\n", userData, requestNumber, size, filename, lineNumber);
				return TRUE;

			case _HOOK_REALLOC:	// - only defined for DEBUG
				//Display the ReAlloc
				InternalTraceFmt(L"TRACE - CRT Realloc - 0x%p, ID=%08lu, %lu byte(s), File: %S, Line: %d\n", userData, requestNumber, size, filename, lineNumber);
				return TRUE;

			case _HOOK_FREE:	// - only defined for DEBUG
				//Display the Free
				InternalTraceFmt(L"TRACE - CRT Free - 0x%p, ID=%08lu, %lu byte(s), File: %S, Line: %d\n", userData, requestNumber, size, filename, lineNumber);
				return TRUE;
