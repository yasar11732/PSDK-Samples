...
HRESULT CStream::Seek(LARGE_INTEGER dlibMove, DWORD dwOrigin, ULARGE_INTEGER* plibNewPosition)
{
	...
	{	
		...
		TESTC(TRACE_METHOD(hr, L"IStream::Seek(%d, %d, &%d)", dlibMove.QuadPart, dwOrigin, plibNewPosition ? plibNewPosition->QuadPart : 0));
	}
...
HRESULT CStream::Stat(STATSTG* pstatstg)
{
	...
	{	
		...
		TESTC(TRACE_METHOD(hr, L"IStream::Stat(&{\"%s\", %d, %d, 0x%08x, 0x%08x, 0x%08x, %d, %d, \"%s\", 0x%08x, %d}, \"%s\")", pstatstg->pwcsName, pstatstg->type, (ULONG)pstatstg->cbSize.QuadPart, &pstatstg->mtime, &pstatstg->ctime, &pstatstg->atime, pstatstg->grfMode, pstatstg->grfLocksSupported, pwszProgID, pstatstg->grfStateBits, pstatstg->reserved, L"STATFLAG_DEFAULT"));
	}
