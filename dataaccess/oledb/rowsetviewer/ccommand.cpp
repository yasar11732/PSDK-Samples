	...
	if(m_pICommandStream)
	{
		...
		if(pwszText)
		{
			...
			if(fUnicode)
			{
				...
				TESTC(pCStream->WriteBuffer(cbWritten, pwszText, wcslen(pwszText)*sizeof(WCHAR), NULL));
			}
			else
			{
				//MBCS-Stream:
				...
				TESTC(pCStream->WriteBuffer(cbWritten, pszText, strlen(pszText)*sizeof(CHAR), NULL));
				SAFE_FREE(pszText);
			}
		}
...
HRESULT CCommand::Execute(CAggregate* pCAggregate, WCHAR* pwszCommandText, REFIID riid, BOOL fUseParams, DBROWCOUNT* pcRowsAffected, IUnknown** ppIUnknown, BOOL fCommandStream)
{
	...
	if(m_pICommand)
	{
		...
		TRACE_METHOD(hr, L"ICommand::Execute(0x%p, %s, 0x%p, &%ld, &0x%p)", pCAggregate, GetInterfaceName(riid), fUseParams ? &m_Parameters.GetParams() : NULL, cRowsAffected, ppIUnknown ? *ppIUnknown : NULL);
