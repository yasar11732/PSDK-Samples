...
HRESULT CFileLite::Write(LPCWSTR pwszText, ULONG* pcbWritten)
{
	...
	if(m_hFile && pwszText)
	{
		if(IsUnicode())
		{
			...
			hr = WriteBytes(wcslen(pwszText)*sizeof(WCHAR), (BYTE*)pwszText, &cbWritten);
		}
		else
		{
			...
			hr = WriteBytes(strlen(szBuffer)*sizeof(CHAR), (BYTE*)szBuffer, &cbWritten);
		}
	}
...
HRESULT CFileLite::Write(LPCSTR pszText, ULONG* pcbWritten)
{
	...
	if(m_hFile && pszText)
	{
		if(IsUnicode())
		{
			...
			hr = WriteBytes(wcslen(wszBuffer)*sizeof(WCHAR), (BYTE*)wszBuffer, &cbWritten);
		}
		else
		{
			//The File is ANSI(MBCS), just directly write out the stream of bytes
			//to MBCS before writting it directly to the file...
			hr = WriteBytes(strlen(pszText)*sizeof(CHAR), (BYTE*)pszText, &cbWritten);
		}
	}
...
/////////////////////////////////////////////////////////////////////
// CFileLite::WriteQuoted
//
/////////////////////////////////////////////////////////////////////
HRESULT CFileLite::WriteQuoted(LPCWSTR pwszText)
{
	if(pwszText == NULL)
	{
		//Simple, empty string within quotes...
		Write(L"\"\"");
	}
	else if(wcschr(pwszText, L'\'')==NULL)
	{
		//String doesn't have embedded single quotes, so we can safely use this for outside
		Write(L"\'");
		Write(pwszText);
		Write(L"\'");
	}
	...
	{
		//String doesn't have embedded double quotes, so we can safely use this for outside
		Write(L"\"");
		Write(pwszText);
		Write(L"\"");	
	}
	else
	{
		...
		//We will need to double the embedded single quotes, so their is no confusion
		//on where the string begins or ends...
		Write(L"\'");
		
		LPCWSTR pwszString = pwszText;
		WCHAR* pwszQuote = wcschr(pwszString, L'\'');
		while(pwszQuote && pwszString[0])
		{
			//Output upto the single quote, and output two single quotes
			*pwszQuote = L'\0';
			Write(pwszString);
			Write(L"\'\'");
			*pwszQuote = L'\'';

			//Move to the next part of the string after what we have already output'ed
			pwszString = pwszQuote + 1;
			pwszQuote = wcschr(pwszString, L'\'');
		}
