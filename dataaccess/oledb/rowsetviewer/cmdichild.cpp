...
//				DisplayDialog(IDD_CREATETABLE, hWnd, CreateTableProc, pCSession, iID);
		...
		case IDM_IROWSETLOCATE_COMPARE:
			if(pCRowset && pCRowset->m_pIRowsetLocate)
			{
				...
				TRACE_METHOD(hr, L"IRowsetLocate::Compare(0x%p, %lu, 0x%p, %lu, 0x%p, &%d)", hChapter, rgcbBookmarks[0], rgpBookmarks[0], rgcbBookmarks[1], rgpBookmarks[1], dwComparison);
		...
		case IDM_IROWSETLOCATE_HASH:
			if(pCRowset && pCRowset->m_pIRowsetLocate)
			{
				...
				TRACE_METHOD(hr, L"IRowsetLocate::Hash(0x%p, %lu, 0x%p, 0x%p, 0x%p, 0x%p)", hChapter, cRows, rgcbBookmarks, rgpBookmarks, rgHashedValues, rgBookmarkStatus);
		...
		case IDM_IROWSETLOCATE_GETROWSBYBOOKMARK:
			if(pCRowset && pCRowset->m_pIRowsetLocate)
			{
				...
				TRACE_METHOD(hr, L"IRowsetLocate::GetRowsByBookmark(0x%p, %lu, 0x%p, 0x%p, 0x%p, 0x%p)", hChapter, cSelItems, rgcbBookmarks, rgpBookmarks, rghRows, rgRowStatus);
			...
			if(pCRowset && pCRowset->m_pIRowsetBookmark)
			{
				...
				if(iSelRow != LVM_ERR && hRow)
				{
					...
					if(SUCCEEDED(hr = pCRowset->GetBookmark(hRow, &cbBookmark, (BYTE**)&pBookmark)))
					{
						...
						TRACE_METHOD(hr, L"IRowsetBookmark::PositionOnBookmark(0x%p, %lu, 0x%p)", hChapter, cbBookmark, pBookmark);
		...
		case IDM_RESYNCHROWS:
			if(pCRowset && pCRowset->m_pIRowsetResynch)
			{
				...
				TRACE_METHOD(hr, L"IRowsetResynch::ResynchRows(%lu, 0x%p, &%lu, &0x%p, &0x%p)", cRows, rghRows, cRowsResynched, rghRowsResynched, rgRowStatus);
		...
		case IDM_REFRESHVISIBLEDATA:
			if(pCRowset && pCRowset->m_pIRowsetRefresh)
			{
				...
				TRACE_METHOD(hr, L"IRowsetRefresh::RefreshVisibleData(0x%p, %lu, 0x%p, %d, &%lu, &0x%p, &0x%p)", hChapter, cRows, rghRows, fOverWrite, cRowsRefreshed, rghRowsRefreshed, rgRowStatus);
...
HRESULT CMDIChild::DeleteSelectedRows()
{
	...
	if(dwSelection == IDYES)
	{
		...
		TESTC(TRACE_METHOD(hr, L"IRowsetChange::DeleteRows(0x%p, %lu, 0x%p, 0x%p)", hChapter, cRows, rghRows, rgRowStatus));
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			if (SUCCEEDED(hr) && cAxis != 0) 
			{
				...
				wSendMessageFmt(::GetDlgItem(hWnd, IDC_DIMENSIONS), WM_SETTEXT, 0, L"%lu", rgAxisInfo->cDimensions);
				wSendMessageFmt(::GetDlgItem(hWnd, IDC_COORDINATES), WM_SETTEXT, 0, L"%lu", rgAxisInfo->cCoordinates);
				...
				case IDCOMBO_AXIS:
				{
					switch(GET_WM_COMMAND_CMD(wParam, lParam))
					{
						...
						{	
							...
							{
								wSendMessageFmt(::GetDlgItem(hWnd, IDC_DIMENSIONS), WM_SETTEXT, 0, L"%lu", rgAxisInfo[iAxis].cDimensions);
								wSendMessageFmt(::GetDlgItem(hWnd, IDC_COORDINATES), WM_SETTEXT, 0, L"%lu", rgAxisInfo[iAxis].cCoordinates);
								...
								for(ULONG iDim=0; iDim<rgAxisInfo[iAxis].cDimensions; iDim++)
								{
									...
									StringFormat(wszBuffer, NUMELE(wszBuffer), L"%lu", rgAxisInfo[iAxis].rgcColumns[iDim]);
									s_listAxisInfo.InsertItem(iDim, 1, wszBuffer);
								}
							}
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			{	
				...
				//Ordinal (SubItem)
				StringFormat(wszBuffer, NUMELE(wszBuffer), L"%ld", pColInfo->iOrdinal);
				...
				//ColumnSize (SubItem)
				StringFormat(wszBuffer, NUMELE(wszBuffer), L"%lu", pColInfo->ulColumnSize);
				...
				case IDOK:
				{
					...
					switch(pThis->m_idSource)
					{
						...
						case IDM_IROWSETLOCATE_GETROWSAT:
						{
							...
							TRACE_METHOD(hr, L"IRowsetLocate::GetRowsAt(0x%p, NULL, %lu, 0x%p, %ld, %ld, &%lu, &0x%p)", pCRowset->m_hChapter, cbBookmark, pBookmark, s_lOffset, s_cRows, cRowsObtained, rghRows);
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			{	
				...
				//Ordinal (SubItem)
				StringFormat(wszBuffer, NUMELE(wszBuffer), L"%ld", pBinding->iOrdinal);
				...
				//cbMaxLen (SubItem)
				StringFormat(wszBuffer, NUMELE(wszBuffer), L"%lu", pBinding->cbMaxLen);
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					TESTC(TRACE_METHOD(hr, L"ICommandWithParameters::SetParameterInfo(%lu, &%ld, 0x%p)", cParams, ulParamOrdinal, &dbParamBindInfo));
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			{	
				...
				//Ordinal (SubItem)
				StringFormat(wszBuffer, NUMELE(wszBuffer), L"%ld", pParamInfo->iOrdinal);
				LV_InsertItem(hWndValue, 0, i, wszBuffer);
				...
				//ulParamSize (SubItem)
				StringFormat(wszBuffer, NUMELE(wszBuffer), L"%lu", pParamInfo->ulParamSize);
				LV_InsertItem(hWndValue, 4, i, wszBuffer);
...
INT_PTR WINAPI CMDIChild::RowOpenProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			INDEX iDefColumn = -1;
			for(ULONG i=0; i<pCRow->m_ColumnInfo.GetCount(); i++)
			{
					...
					switch(GET_WM_COMMAND_ID(wParam, lParam))
					{
						...
						case IDC_TYPENAME:
						{
							...
							if (iSelTypeName >= 0)
							{
								...
								StringFormat(wszBuffer, NUMELE(wszBuffer), L"%lu", pCSession->m_rgProvTypes[iSelTypeName].ulColumnSize);
...
INT_PTR WINAPI CMDIChild::CreateTableProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		...
		{	
			...
			// Type Name list...
			for(ULONG i=0; i<pCSession->m_cProvTypes; i++)
			{
				//Type Name
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					for(INDEX iSel = 0; iSel < nCount; iSel++)
					{
					...
					TESTC(TRACE_METHOD(hr, L"IScopedOperations::Delete(%lu, 0x%p, 0x%08x, 0x%p)", nCount, rgpwszURLs, dwDeleteFlags, rgdbStatus));
				...
				CLEANUP:
					...
					for(iSel=0; iSel<nCount; iSel++)
					{
						...
						if(rgdbStatus[iSel] != DBSTATUS_S_OK)
							wMessageBox(GetFocus(), MB_OK, wsz_ERROR, L"Status = %s \nrgpwszURL[%lu] = %s", GetStatusName(rgdbStatus[iSel]), iSel, rgpwszURLs[iSel]);
					}
				...
				case IDOK:
				{
					...
					switch(pThis->m_idSource)
					{
						case IDM_ISCOPEDOPERATIONS_COPY:
							...
							TESTC(TRACE_METHOD(hr, L"IScopedOperations::Copy(%lu, 0x%p, 0x%p, 0x%08x, NULL, 0x%p, 0x%p, &0x%p)", cRows, rgpwszSourceURLs, rgpwszDestURLs, dwFlags, rgdbStatus, rgpwszNewURLs, pStringsBuffer));
							break;
						...
						case IDM_ISCOPEDOPERATIONS_MOVE:
							...
							TESTC(TRACE_METHOD(hr, L"IScopedOperations::Move(%lu, 0x%p, 0x%p, 0x%08x, NULL, 0x%p, 0x%p, &0x%p)", cRows, rgpwszSourceURLs, rgpwszDestURLs, dwFlags, rgdbStatus, rgpwszNewURLs, pStringsBuffer));
							break;
					};
...
HRESULT CMDIChild::GetChapteredChild(INDEX iSelectedCol, const DBBINDING* pBinding, REFIID riid)
{
	...
	if(pCRowset && pCRowset->m_pIRowsetInfo)
	{
		...
		TESTC(TRACE_METHOD(hr, L"IRowsetInfo::GetReferencedRowset(%ld, %s, &0x%p)", pBinding->iOrdinal, GetInterfaceName(riid), pIUnknown));
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			for(ULONG i=0; i<pCRowset->m_Bindings.GetCount(); i++)
			{
				...
				{
					iSel = (INDEX)wSendMessageFmt(hWndColumn, CB_ADDSTRING,	0, L"%2d - %s <ISCHAPTER>", iOrdinal, GetColName(pColInfo));
					if(iSavedCol == CB_ERR)
						iSavedCol = iSel;
				}
				else
				{
					iSel = (INDEX)wSendMessageFmt(hWndColumn, CB_ADDSTRING,	0, L"%2d - %s", iOrdinal, GetColName(pColInfo));
				}
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					TESTC(TRACE_METHOD(hr, L"IRowsetFind::FindNextRow(0x%p, 0x%p, \"%s\", 0x%08x, %lu, 0x%p, %ld, %ld, &%lu, &0x%p)", hChapter, hAccessor, pwszValue, dwCompareOps, cbBookmark, pBookmark, lOffset, cSavedRows,  cRowsObtained, rghRows));
...
HRESULT CDataGrid::DisplayColumnInfo()
{
	...
	for(ULONG i=0; i<pCDataAccess->m_Bindings.GetCount(); i++)
	{
