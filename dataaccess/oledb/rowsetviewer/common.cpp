...
WCHAR* StringCopy(WCHAR* pwszDest, LPCWSTR pwszSource, DBLENGTH cMaxLen)
{
	...
	WCHAR* pwszReturn = wcsncpy(pwszDest, pwszSource, (size_t)cMaxLen);
	if(cMaxLen)
	{
		pwszDest[cMaxLen-1] = wEOL;
	}
...
WCHAR* StackFormat(LPCWSTR pwszFmt, ...)
{
	...
	_vsnwprintf(wszBuffer, NUMELE(wszBuffer), pwszFmt, marker);
	va_end(marker);
...
WCHAR* StringFormat(WCHAR* pwszDest, DBLENGTH ulStrLen, LPCWSTR pwszFmt, ...)
{
	...
	//upto the max size of the array
	va_start(marker, pwszFmt);
	_vsnwprintf(pwszDest, ulStrLen, pwszFmt, marker);
	va_end(marker);
...
{
	//Safe guard against overflow...
	CHAR* pszReturn = strncpy(pszDest, pszSource, (size_t)cMaxLen);
	if(cMaxLen)
	{
		pszDest[cMaxLen-1] = EOL;
	}
...
//			StringFormat(pwsz, ulMaxSize, L"%ld", V_I8(pVariant));
			StringFormat(pwsz, ulMaxSize, L"%ld", pVariant->ullVal);
			break;
		}
...
HRESULT CreateDefFileName(LPCWSTR pwszFileName, WCHAR* pwszFullPath, ULONG ulMaxSize, BOOL fCurrentDir)
{
	...
	if(ulStrLen)
	{
		...
		wcscat(pwszFullPath, pwszFileName);
		return S_OK;
	}	
