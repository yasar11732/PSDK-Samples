...
HRESULT		CDataSource::OpenFile(WCHAR* pwszFileName, CFileIO** ppCFileIO)
{
	ASSERT(ppCFileIO);
	HRESULT hr = S_OK;
	WCHAR	wszFullPath[_MAX_PATH + _MAX_PATH + 1]={0};
	CHAR	szFullPath[_MAX_PATH + _MAX_PATH + 1]={0};
	...
	if(m_wszPath[0])
	{
		...
		wcsncat(wszFullPath, m_wszPath, wcslen(m_wszPath));
		wcsncat(wszFullPath, L"\\", wcslen(L"\\"));
		wcsncat(wszFullPath, pwszFileName, 
				(_MAX_PATH + _MAX_PATH - wcslen(wszFullPath)));
