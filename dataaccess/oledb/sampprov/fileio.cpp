    ...
    )
{
    ...
    // Open the File
	open(ptstrFileName, ios::in);
	if(!is_open())
	{
		return E_FAIL;  // The file does not exist.  we should not create it.
	}
	close();
	open( ptstrFileName, ios::in | ios::out );
    if( !is_open() ) {
		close();
		open( ptstrFileName, ios::in );
		m_FileReadOnly = TRUE;
    ...
    )
{
    ...
    case TYPE_CHAR:
        ...
        m_rgpColumnData[cColumn]->dwLength = lstrlen( (LPTSTR) m_rgpColumnData[cColumn]->bData );
        m_rgpColumnData[cColumn]->dwStatus = DBSTATUS_S_OK;
        break;

    case TYPE_SLONG:
        *(ULONG*) m_rgpColumnData[cColumn]->bData = atol( pvCopy );
        m_rgpColumnData[cColumn]->dwLength = 4;
        m_rgpColumnData[cColumn]->dwStatus = DBSTATUS_S_OK;
        break;
    ...
    for (cCols = 1; cCols <= m_cColumns; cCols++)
        ...
        if (pColData->dwStatus != DBSTATUS_S_ISNULL)
            ...
            switch (m_rgswColType[cCols])
                {
            case TYPE_CHAR:
				strncpy( szTmpBuff, (LPSTR)pColData->bData, pColData->dwLength );
				szTmpBuff[pColData->dwLength] = '\0';
                nCnt = wsprintf( pvInput, "\"%s\"", szTmpBuff );
                break;
