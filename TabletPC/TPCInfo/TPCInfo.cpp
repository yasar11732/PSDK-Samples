...
{    
    ...
    if (RegOpenKeyExW(hKeyRoot, L"Version", 0, KEY_READ, &hKey) == ERROR_SUCCESS) 
    {
        ...
        if (iLen < iVersionMaxLen)
        {
            ...
            lstrcpynW(info.wchVersion, pcwsVersion, iLen + 1);
                            ...
                            && (NULL != pwchBuildVer))
                        {
                            ...
                            if (0 < iLen)
                            {
                                if (iLen < iVersionMaxLen)
                                {
                                    ...
                                    lstrcpynW(info.wchVersion + iLen, pcwsBuild, 
                                              iVersionMaxLen - iLen);
                                    iLen += lstrlenW(pcwsBuild);
                                    if (iLen < iVersionMaxLen)
                                    {
                                        lstrcpynW(info.wchVersion + iLen, pwchBuildVer, 
                                                  iVersionMaxLen - iLen);
                                    }
                                }
                            }
                            ...
                            {
                                lstrcpynW(info.wchVersion, pwchBuildVer, iVersionMaxLen);
                            }
                            info.wchVersion[iVersionMaxLen-1] = 0;
                        }
                    }
