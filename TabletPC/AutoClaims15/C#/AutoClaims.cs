...
//   - PenInputPanel (collecting handwriting in an attached window)
...
namespace AutoClaims
{
    ...
    {
        // Win32 Error code REGDB_E_CLASSNOTREG
        ...
        // Declare PenInputPanel objects (used to collect handwriting 
        // for the edit controls on the form)...
        // pipShared is shared between the Year, Make, and Model fields
        PenInputPanel pipShared = null; 
        // pipPolicyNumber is used for the Policy Number field
        PenInputPanel pipPolicyNumber = null;
        // pipName is used for the Insured Name field
        PenInputPanel pipName = null;
        ...
        protected override void Dispose( bool disposing )
        {
            if( disposing )
            {
                ...
                if (pipPolicyNumber != null)
                {
                    pipPolicyNumber.Dispose();
                }
                if (pipName != null)
                {
                    pipName.Dispose();
        ...
        private void AutoClaims_Load(object sender, System.EventArgs e)
        {
            ...
            // Initialize the PenInputPanel objects inside a Try/Catch block
            // since the objects are not supported off a Tablet PC.
            try 
            {
                // Initialize the PenInputPanel objects.
                // - pipShared is attached dynamically at run-time
                // - pipPolicyNumber is only attached to the Policy Number field
                // - pipName is only attached to the Insured Name field
                ...
                // Enable Text Services Framework on the Name and Policy Number fields.
                // InkEdit already provides a correction UI, but here EnableTsf is allowing 
                // the PenInputPanel to use the TSF insertion context rather than SendInput
                // to send the handwriting recognition results into the control. This means 
                // that text can be inserted even if the field no longer has focus.
                pipName.EnableTsf(true);
                pipPolicyNumber.EnableTsf(true);

                // Since the fields are laid out vertically on the form, we'll
                // display the PenInputPanels slightly further to the right
                // in order to make it easier for the user to navigate.
        ...
        private void inkEdYear_Enter(object sender, System.EventArgs e)
        {
            if (pipShared != null)
            {
                ...
                // Enable Text Services Framework on the Year field.
                // InkEdit already provides a correction UI, but here EnableTsf is allowing 
                // the PenInputPanel to use the TSF insertion context rather than SendInput
                // to send the handwriting recognition results into the control. This means 
                // that text can be inserted even if the field no longer has focus.
        ...
        private void inkEdMake_Enter(object sender, System.EventArgs e)
        {
            if (pipShared != null)
            {
                ...
                // Enable Text Services Framework on the Make field.
                // InkEdit already provides a correction UI, but here EnableTsf is allowing 
                // the PenInputPanel to use the TSF insertion context rather than SendInput
                // to send the handwriting recognition results into the control. This means 
                // that text can be inserted even if the field no longer has focus.
        ...
        private void inkEdModel_Enter(object sender, System.EventArgs e)
        {
            if (pipShared != null)
            {
                ...
                // Enable Text Services Framework on the Model field.
                // InkEdit already provides a correction UI, but here EnableTsf is allowing 
                // the PenInputPanel to use the TSF insertion context rather than SendInput
                // to send the handwriting recognition results into the control. This means 
                // that text can be inserted even if the field no longer has focus.
