...
//      IInkRecognitionResult, IInkRecognitionGuide, IInkWordList, IInkGesture
...
// The following combinations of factoids are valid, though not defined in msinkaut.h
// (order of the combined factoid names and the number of spaces is not important)
// FACTOID_EMAIL | FACTOID_WORDLIST 
#define FACTOID_EMAIL_WORDLIST          L"EMAIL | WORDLIST"     
// FACTOID_WEB | FACTOID_WORDLIST
#define FACTOID_WEB_WORDLIST            L"WEB | WORDLIST"
// FACTOID_WEB | FACTOID_FILENAME | FACTOID_WORDLIST
#define FACTOID_WEB_FILENAME_WORDLIST   L"WEB | FILENAME | WORDLIST"

// The names of the supported factoids. 
// All these names are used both to create a menu item and 
// as the parameter in IInkRecoContext::put_Factoid(name).
const LPOLESTR gc_pwsFactoids[] = { 
        FACTOID_DEFAULT, FACTOID_NONE, FACTOID_SYSTEMDICTIONARY, FACTOID_WORDLIST,
        FACTOID_CURRENCY, FACTOID_DATE, FACTOID_DIGIT, 
        FACTOID_EMAIL, FACTOID_EMAIL_WORDLIST, FACTOID_WEB, 
        FACTOID_WEB_WORDLIST, FACTOID_FILENAME, FACTOID_WEB_FILENAME_WORDLIST,
        FACTOID_NUMBER, FACTOID_ONECHAR, FACTOID_UPPERCHAR, 
        FACTOID_PERCENT, FACTOID_TELEPHONE,FACTOID_TIME, FACTOID_POSTALCODE,        
        FACTOID_BOPOMOFO, FACTOID_CHINESESIMPLECOMMON, FACTOID_CHINESETRADITIONALCOMMON, 
        FACTOID_HANGULCOMMON, FACTOID_HIRAGANA, FACTOID_JAMO, FACTOID_JAPANESECOMMON,
        FACTOID_KANJICOMMON, FACTOID_KATAKANA, FACTOID_KOREANCOMMON 
};

// The following array of indices to the gc_pwsFactoids makes the subset 
// of factoids which can be used with a custom wordlist.
const UINT gc_nWordlistFactoids[] = {
        0 /*FACTOID_DEFAULT*/, 3 /*FACTOID_WORDLIST*/, 8 /*FACTOID_EMAIL_WORDLIST*/, 
        10 /*FACTOID_WEB_WORDLIST*/, 12 /* FACTOID_WEB_WORDLIST_FILENAME */
};
        ...
        )
{
                      ...
                      WS_OVERLAPPEDWINDOW, 0, (UINT)hMenu) != NULL)
    {
        ...
        // Set factoid to default
        theApp.SendMessage(WM_COMMAND, ID_FACTOID_FIRST);  
        ...
        )
{
    ...
    // Create a wordlist object and populate it from the application's
    // string table resource
    if (SUCCEEDED(m_spIInkWordList.CoCreateInstance(CLSID_InkWordList)))
    {
        TCHAR buffer[100]; // use buffer large enough to load the strings;
                           // even if it's not, LoadString will return non-zero
                           // and the string won't be added to the wordlist
        for (int i = IDS_WORD_FIRST; i <= IDS_WORD_LAST; i++)
        {
            if (::LoadString(_Module.GetResourceInstance(), i, buffer, 
                             countof(buffer)) != 0)
            {
                CComBSTR bstr(buffer);
                m_spIInkWordList->AddWord(bstr);
            }
        }
    }
...
/////////////////////////////////////////////////////////
//                                          
// CAdvRecoApp::OnFactoidWordlist
//  
// This command handler is called when user checks on or off
// the "Use Wordlist" menu item in the "Factoid" submenu.
//
// Parameters:
...
//
/////////////////////////////////////////////////////////
LRESULT CAdvRecoApp::OnFactoidWordlist(
        WORD /*wNotifyCode*/, 
        WORD wID, 
        HWND /*hWndCtl*/, 
        BOOL& /*bHandled*/
...
{
    // Ignore the command if either the recognition context 
    // or the wordlist has not been created
    if (m_spIInkRecoContext == NULL || m_spIInkWordList == NULL)
        return 0;
    ...
    if (TRUE == m_bUseWordList)
    {
        // Reset the WordList property of the recognition context to NULL
        if (FAILED(m_spIInkRecoContext->putref_WordList(NULL)))
        {
            MessageBox(TEXT("Failed to reset the Wordlist property!"), 
                       gc_szAppName, MB_ICONERROR | MB_OK);
            return 0;
        }
        m_bUseWordList = false;
    }
    else
    {
        // Pass the pointer to the InkWordList object to the recognition context
        HRESULT hr = m_spIInkRecoContext->putref_WordList(m_spIInkWordList);
        if (FAILED(hr))
        {
            if (E_NOTIMPL == hr)
            {
                MessageBox(TEXT("The recognizer doesn't support wordlists."), 
                           gc_szAppName, MB_ICONINFORMATION | MB_OK);
            }
            else
            {
                MessageBox(TEXT("Failed to set the Wordlist property!"), 
                           gc_szAppName, MB_ICONERROR | MB_OK);
            }
            return 0;
        }
    ...
    if (NULL != hMenu)
    {
        HMENU hSubMenu = ::GetSubMenu(hMenu, mc_iSubmenuFactoids);
        if (NULL != hSubMenu)
        {
            ::CheckMenuItem(hSubMenu, wID, 
                            MF_BYCOMMAND | (m_bUseWordList ? MF_CHECKED : MF_UNCHECKED));
        }
    }
...
/////////////////////////////////////////////////////////
//                                          
// CAdvRecoApp::OnFactoid
//  
// This command handler is called when user selects 
// a factoid name in the "Factoid" submenu.
//
// Parameters:
//      defined in the ATL's macro COMMAND_RANGE_HANDLER
//      Only wID - the id of the command associated 
...
{
    UINT iFactoid = wID - ID_FACTOID_FIRST;     // get the index to gc_pwsFactoids
    // Return if user clicks on the currently selected factoid menu item or if 
    // the command id is not in the valid range (shouldn't happen with error free code) 
    if (wID == m_nCmdFactoid || iFactoid >= countof(gc_pwsFactoids))
        return 0;

    // Tell the context to use the factoid, by assigning the factoid name 
    ...
    if (m_spIInkRecoContext != NULL)
    {
        ...
        CComBSTR bstrFactoid(gc_pwsFactoids[iFactoid]);
        HRESULT hr = m_spIInkRecoContext->put_Factoid(bstrFactoid);
        ...
        else if (TPC_E_INVALID_PROPERTY == hr) // the factoid is not supported
        {
    ...
    // Update the menu and the status bar
    UpdateMenuRadioItems(mc_iSubmenuFactoids, wID, m_nCmdFactoid);
    m_nCmdFactoid = wID;
    UpdateStatusBar();
...
// Also, it fills the Factoid menu with the items for the supported 
// factoids.
//
// Parameters:
//      none
...
HMENU CAdvRecoApp::LoadMenu()
{
    ...
    // Get the Factoid submenu and create command items 
    // for each factoid name defined in gc_pwsFactoids[]
    hSubMenu = ::GetSubMenu(hMenu, mc_iSubmenuFactoids);
    if (hSubMenu)
    {
        // The ID_FACTOID_FIRST is defined in the resource.h file. 
        // It's the base id for the commands that fill this submenu
        miinfo.wID = ID_FACTOID_FIRST;
        miinfo.fState = 0;
        lCount = countof(gc_pwsFactoids);
        for(LONG i = 0; i < lCount; i++, miinfo.wID++)
        {
            miinfo.dwTypeData = gc_pwsFactoids[i];
            ::InsertMenuItemW(hSubMenu, (UINT)-1, TRUE, &miinfo);
        }
    }
...
// a different recognizer, factoid or guide mode.
//        
// Parameters:
        ...
        )
{
    ...
    if (NULL != hMenu)
    {
        ...
        if (NULL != hSubMenu)
        {
            ...
            // Update the "Use Wordlist" menu item, if the change is in the factoid menu
            if (mc_iSubmenuFactoids == iSubMenu)
            {
                // Enable the menu item if the selected factoid can be combined with a wordlist
                UINT uFlags = MF_BYCOMMAND | MF_GRAYED;
                for (int i = 0; i < countof(gc_nWordlistFactoids); i++)
                {
                    if ((idCheck - ID_FACTOID_FIRST) == gc_nWordlistFactoids[i])
                    {
                        uFlags &= ~MF_GRAYED;
                        uFlags |= MF_ENABLED;
                        break;
                ...
                ::EnableMenuItem(hSubMenu, ID_FACTOID_WORDLIST, uFlags);
            }
        }
    }
}
        ...
        )
{
    ...
    // Set the wordlist
    if (true == m_bUseWordList && m_spIInkWordList != NULL)
    {
        hr = spNewContext->putref_WordList(m_spIInkWordList);
        if (FAILED(hr))
        {
            if (E_NOTIMPL == hr)
            {
                // The recognizer doesn't support wordlists
            }
            else
            {
                MessageBox(TEXT("Failed to set wordlist to the new recognition context!"), 
                           gc_szAppName, MB_ICONERROR | MB_OK);
            }

            // Uncheck the "Use Wordlist" menu item
            m_bUseWordList = false; 
            if (NULL != hMenu)
            {
                hSubMenu = ::GetSubMenu(hMenu, mc_iSubmenuFactoids);
                if (NULL != hSubMenu)
                {
                    ::CheckMenuItem(hSubMenu, ID_FACTOID_WORDLIST, MF_BYCOMMAND | MF_UNCHECKED);
                }
            }
        }
    }
    ...
    // Set the factoid
    UINT iFactoid = m_nCmdFactoid - ID_FACTOID_FIRST;
    if (iFactoid < countof(gc_pwsFactoids))
    {
        CComBSTR bstrFactoid(gc_pwsFactoids[iFactoid]);
        hr = spNewContext->put_Factoid(bstrFactoid);
        if ((TPC_E_INVALID_PROPERTY == hr) && (0 != iFactoid))  
        {
            // This factoid is not supported by the recognizer. Set the default value
            bstrFactoid = gc_pwsFactoids[0];
            hr = spNewContext->put_Factoid(bstrFactoid);
            if (SUCCEEDED(hr))
            {
                UpdateMenuRadioItems(mc_iSubmenuFactoids, ID_FACTOID_FIRST, m_nCmdFactoid);
                m_nCmdFactoid = ID_FACTOID_FIRST;
            }
        }
        if (FAILED(hr))
        {
...
bool CAdvRecoApp::CreateChildWindows()
{
    ...
    lvItem.iSubItem = 0;
    for (int i = 0; i < mc_cNumSSGestures; i++)
    {
        lvItem.iItem = i;
...
// recognizer and factoid to the application's status bar. 
//
// Parameters:
//      none
//
...
void CAdvRecoApp::UpdateStatusBar()
{
    ...
    UINT iFactoid = m_nCmdFactoid - ID_FACTOID_FIRST;
    if (iFactoid >= sizeof(gc_pwsFactoids)/sizeof(gc_pwsFactoids[0]))
    {
        iFactoid = 0;
    }

    CComBSTR bstrStatus(m_bstrCurRecoName);
    bstrStatus += L"; factoid: ";
    bstrStatus += gc_pwsFactoids[iFactoid];
