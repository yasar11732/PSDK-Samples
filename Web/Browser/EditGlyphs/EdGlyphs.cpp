...
// EdGlyphs.cpp : Implementation of WinMain

// Note: Proxy/Stub Information
// To build a separate proxy/stub DLL,
// run nmake -f EdGlyphsps.mk in the project directory.
								...
								int /*nShowCmd*/)
{
	...
	_Module.Init(ObjectMap, hInstance, &LIBID_EDGLYPHSLib);
	...
	while (lpszToken != NULL)
	{
		...
		{
			_Module.UpdateRegistryFromResource(IDR_EdGlyphs, FALSE);
			hr = _Module.UnregisterServer(TRUE);
			bRun = FALSE;
			//if (SUCCEEDED(hr)) MessageBox(NULL, _T("Unregistration Successful"), NULL, NULL);
			break;
		}

		if (lstrcmpi(lpszToken, _T("RegServer"))==0)
		{
			_Module.UpdateRegistryFromResource(IDR_EdGlyphs, TRUE);
			hr = _Module.RegisterServer(TRUE);
			bRun = FALSE;
			//if (SUCCEEDED(hr)) MessageBox(NULL, _T("Registration Successful"), NULL, NULL);
			break;
		}
	...
	if (bRun)
	{
		...
		if (pApp)
		{
			...
			while (GetMessage(&msg, 0, 0, 0))
			{
				...
				// If we get a keydown message, we must send the message
