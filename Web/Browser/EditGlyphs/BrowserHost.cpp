...
LRESULT CBrowserHost::OnButton2(WORD, WORD, HWND, BOOL&)
{
	...
	// If button is up, empty glyph table and return
	if (!m_bButton2Down)
	{
		V_VT(&var) = VT_BOOL;
		V_BOOL(&var) = VARIANT_TRUE;
	...
	bstrGlyphTable.AppendBSTR(bstrTemp);
	bstrTemp.LoadString(IDS_H3BGN_GLYPHSTRING);
	bstrGlyphTable.AppendBSTR(bstrTemp);
	bstrTemp.LoadString(IDS_H3END_GLYPHSTRING);
	bstrGlyphTable.AppendBSTR(bstrTemp);
	bstrTemp.LoadString(IDS_H3ABS_GLYPHSTRING);
	bstrGlyphTable.AppendBSTR(bstrTemp);
	bstrTemp.LoadString(IDS_H4_GLYPHSTRING);
	bstrGlyphTable.AppendBSTR(bstrTemp);
...
STDMETHODIMP CBrowserHost::Run()
{
	...
	// Load Test Page
	m_spWebBrowser->Navigate(L"res://edglyphs.exe/test.html", NULL, NULL, NULL, NULL);
