...
{
	static const IID* arr[] = { &IID_IAccount	};
...
STDMETHODIMP CAccount::Post(long lngAccntNum, long lngAmount, BSTR *pVal)
{
	...
	hr = pConnection->Open(CONNECTION, NULL, NULL, NULL);
	...
	wsprintf(strErr, "Update Account Set Balance = Balance + %d where AccountNo = %d", lngAmount, lngAccntNum);

	bstrSQL = CComBSTR(strErr);

TryAgain:	
	
	hr = pConnection->Execute(bstrSQL, static_cast<VARIANT *>(&vtMissing), adCmdText, &pRecordset);
	...
	// If the current balance is less than 0, abort transaction otherwise complete transaction
	...
	wsprintf(strErr, "%s account %ld, balance is $%ld. (VC++)", ((lngAmount >= 0) ? "Credit to" : "Debit from"), lngAmount, vntValue.lVal);
...
ErrorCreateTable:
	...
	hr = pObjectContext->CreateInstance(CLSID_CreateTable, IID_ICreateTable, (void**)&pCreateTable);
