...
BOOL ISAPITHREAD::InitThreadPool(LPTHREAD_START_ROUTINE pThreadProc, DWORD dwNumThreads, DWORD dwQueueSize)
{
	DWORD dwThreadId;
	...
	m_dwWorkItemQueueSize = (dwQueueSize == 0xffffffff) ? dwNumThreads * 2 : dwQueueSize;
	...
	for (DWORD x = 0; x < dwNumThreads; x++)
	{
...
BOOL ISAPITHREAD::QueueWorkItem(EXTENSION_CONTROL_BLOCK *pecb, BOOL fReleaseThread)
{
	...
	DWORD dwInsertionPoint = m_dwCurrentItem + m_dwItemsInQueue;
...
EXTENSION_CONTROL_BLOCK * ISAPITHREAD::GetWorkItem(void)
{
	EnterCriticalSection(&m_csQueue);
	...
	EXTENSION_CONTROL_BLOCK *pRet = m_WorkItemQueue[m_dwCurrentItem];
...
void ISAPITHREAD::ReleaseThread(DWORD dwNumThreads)
{
...
void ISAPITHREAD::ClearQueue(void)
{
	EnterCriticalSection(&m_csQueue);

	// Clear the work items in the queue

	for (DWORD x = 0; x < m_dwWorkItemQueueSize; x++)
...
ISAPITHREAD::~ISAPITHREAD(void)
{
	...
	for (DWORD x = 0; x < m_dwNumThreads; x++)
