...
DllMain(IN HINSTANCE hinstDll, IN DWORD fdwReason, IN LPVOID lpvContext OPTIONAL)
{
	BOOL fReturn = TRUE;
...
BOOL WINAPI GetExtensionVersion(HSE_VERSION_INFO *pVer)
{
	...
	lstrcpyn((LPSTR) pVer->lpszExtensionDesc, "Asynchronous Read Client Sample ISAPI DLL", HSE_MAX_EXT_DLL_NAME_LEN);
...
{   
	...
	*/
	...
	if (piowi->pecb->cbTotalBytes == piowi->pecb->cbAvailable) {
		...
		wsprintf(pbData, g_szReport, piowi->pecb->cbTotalBytes, piowi->pecb->cbAvailable);
