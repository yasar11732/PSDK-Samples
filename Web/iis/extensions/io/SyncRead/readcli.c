...
BOOL WINAPI GetExtensionVersion(HSE_VERSION_INFO *pVer)
{
	...
	lstrcpyn((LPSTR) pVer->lpszExtensionDesc, "ReadClient Extension", HSE_MAX_EXT_DLL_NAME_LEN);
...
DWORD WINAPI HttpExtensionProc(LPEXTENSION_CONTROL_BLOCK pec)
{
	...
	if (NULL == szBuffer) {
		...
		sprintf(szTemp, "Unable to allocate %ld bytes of memory for buffer", dwContentLength + 5);
