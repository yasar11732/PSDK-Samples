	...
	if (hFile == INVALID_HANDLE_VALUE) {
		...
		if (hseStatus == HSE_STATUS_SUCCESS) {
			...
			if (hseStatus != HSE_STATUS_PENDING) {
				...
				/* assume that the data is transmitted, and close file handle */
...
DWORD ReadDataForPaw(IN PAWI paw)
{
	DWORD hseStatus = HSE_STATUS_SUCCESS;

	/* read data from the file */

	if (!FcReadFromFile(paw->hFile, paw->pchBuffer, MAX_BUFFER_SIZE, &paw->nRead, &paw->ov))
		hseStatus = HSE_STATUS_ERROR;

	return hseStatus;
}

DWORD SendDataForPaw(IN PAWI paw)
{
	DWORD hseStatus = HSE_STATUS_PENDING;

	/* write data to client */

	if (!paw->pecb->WriteClient(paw->pecb->ConnID, paw->pchBuffer, &paw->nRead, HSE_IO_ASYNC))
		hseStatus = HSE_STATUS_ERROR;
...
DWORD SendFileOver(IN EXTENSION_CONTROL_BLOCK *pecb, IN HANDLE hFile)
{
	...
	if (!pecb->ServerSupportFunction(pecb->ConnID, HSE_REQ_IO_COMPLETION, HseIoCompletion, 0, (LPDWORD)paw)) {
