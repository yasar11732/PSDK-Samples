...
DWORD WINAPI HttpExtensionProc(IN EXTENSION_CONTROL_BLOCK *pECB)
{
	...
	if (GetQueryStringField(pECB->lpszQueryString, "chunksize", szChunkSize, sizeof szChunkSize))
	...
	if (!GetQueryStringField(pECB->lpszQueryString, "file", szPath, cchPath)) {
	...
	if (!pECB->ServerSupportFunction(pECB->ConnID, HSE_REQ_MAP_URL_TO_PATH,	szPath,	&cchPath,	NULL))
		return HSE_STATUS_ERROR;
	...
	GetFileMimeType(szPath, szHeaders + cchHeaders, sizeof szHeaders - cchHeaders);
...
BOOL WINAPI GetExtensionVersion(OUT HSE_VERSION_INFO *pVer)
{
	...
	lstrcpyn(pVer->lpszExtensionDesc, "ISAPI CTE test", HSE_MAX_EXT_DLL_NAME_LEN);
...
BOOL WINAPI DllMain(IN HINSTANCE hInstance, IN DWORD fdwReason, IN LPVOID lpvReserved)
{
...
static BOOL SendChunkedFile(EXTENSION_CONTROL_BLOCK *pECB, DWORD dwChunkSize, LPCSTR pszPath)
{
		...
		if (hCteWrite) {
			...
			for (;;) {
				...
				if (!ReadFile(hFile, buf, sizeof buf, &cbread, NULL)) {
					...
					/* if ReadFile fails, break out of loop and cause the function to return FALSE (failure) */
...
/*
	...
	Arguments:
		...
		pszHeaders - any additional headers, separated by CRLFs and terminated by empty line
		fKeepConn - specifies whether to keep TCP connection open or close it after request is processed.
...
static BOOL SendHttpHeaders(EXTENSION_CONTROL_BLOCK *pECB, LPCSTR pszStatus, LPCSTR pszHeaders, BOOL fKeepConn)
{
...
static BOOL GetFileMimeType(LPCSTR pszPath, LPSTR pszType, DWORD cbMax)
{
	HKEY hKey;
	DWORD value_type;
	LONG result;
	...
	*pszType = '\0';
	...
	if (pszExt != NULL) {
		...
		/* for file extension .xyz, MIME Type can be found HKEY_CLASSES_ROOT\.xyz key in the registry */

		result = RegOpenKeyEx(HKEY_CLASSES_ROOT, pszExt, 0L, KEY_READ, &hKey);
	                
		if (result == ERROR_SUCCESS) {
	  
			/* we sucessfully opened the key. try getting the "Content Type" value */
	    
			result = RegQueryValueEx(hKey, "Content Type", NULL, &value_type, (BYTE *)pszType, &cbMax);

			/* if we failed to get the value or it is not string, clear content-type field */
...
static void DisplayExampleUsage(EXTENSION_CONTROL_BLOCK *pECB)
{
	static char szUsage[] = 
		"Example usage:\r\n"
	...
	DWORD dwLength = sizeof szUsage - 1;

	sprintf(szHeaders, "Content-Length: %u\r\nContent-Type: text/plain\r\n\r\n", dwLength);

	if (SendHttpHeaders(pECB, "200 OK", szHeaders, FALSE))
		pECB->WriteClient(pECB->ConnID, szUsage, &dwLength, HSE_IO_SYNC);
}

/*
	Description:

		Assuming "key1=value1+key2=value2" syntax, extract the value for specified key.
...
BOOL GetQueryStringField(LPCSTR pszQueryString, LPCSTR pszKey, LPSTR buf, DWORD cbuf)
{
	...
	/* copy the value up to: the end of line, cbuf chars, or '+' separator, whichever comes first */
