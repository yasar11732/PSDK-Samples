...
#define CTE_ENCODER_SIZE(dwChunkSize) (sizeof(CTE_ENCODER) + dwChunkSize + CTE_MAX_ENCODING_OVERHEAD)
...
HCTE_ENCODER CteBeginWrite(IN EXTENSION_CONTROL_BLOCK *pECB, IN DWORD dwChunkSize)   
{
	HCTE_ENCODER h;
	...
	if (h != NULL) {
		...
		/* chunk data bytes follow the context structure itself and chunk header */

		h->pData = (BYTE *) h + sizeof( *h ) + CTE_MAX_CHUNK_HEADER_SIZE;

		/* this is the CRLF which follows chunk size (and immediately precedes data) */
...
/*
	Description: 
		...
		Send one chunk of data using ClientWrite() <hex encoded chunk size>, CRLF, data bytes, if any, CRLF
...
static BOOL CteSendChunk(IN HCTE_ENCODER h)
{
	...
	/* produce hex string of the number of bytes and compute the length of this string */

	_itoa(h->cbData, szChunkLength, 16); 

	cbChunkLength = strlen(szChunkLength);

	/* step back to make place for hex number and CRLF, copy hex string to its location */
	
	buf = h->pData - 2 - cbChunkLength;

	memmove(buf, szChunkLength, cbChunkLength);

	/* compute the number of bytes to send (this includes chunk data size, hex string and CRLF) */
	...
	success = h->pECB->WriteClient(h->pECB->ConnID, buf, &cbToSend, HSE_IO_SYNC);
...
BOOL CteWrite(IN HCTE_ENCODER h, IN PVOID pData, IN DWORD cbData)
{
	DWORD cbToConsume;
	...
	for (;;) {
		...
		/* compute the number of bytes to consume, break out of the loop, if nothing is left */
...
BOOL CteEndWrite(IN HCTE_ENCODER h)
{
	BOOL success;
