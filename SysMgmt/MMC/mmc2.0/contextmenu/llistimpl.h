...
template<class TYPE, class ARG_TYPE>
typename CLListImpl<TYPE, ARG_TYPE>::CNode*
CLListImpl<TYPE, ARG_TYPE>::NewNode(typename CLListImpl::CNode* pPrev, typename CLListImpl::CNode* pNext)
{
	if( NULL == m_pNodeFree )
	{
		// add another block
...
template<class TYPE, class ARG_TYPE>
void CLListImpl<TYPE, ARG_TYPE>::FreeNode(typename CLListImpl::CNode* pNode)
{
	(pNode->data).~TYPE();
	pNode->pNext = m_pNodeFree;
	m_pNodeFree = pNode;
